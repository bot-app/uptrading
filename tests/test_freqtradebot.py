...
from uptrading.constants import CANCEL_REASON, UNLIMITED_STAKE_AMOUNT
from uptrading.enums import (CandleType, ExitCheckTuple, ExitType, RPCMessageType, RunMode,
                             SignalDirection, State)
from uptrading.exceptions import (DependencyException, ExchangeError, InsufficientFundsError,
                                  InvalidOrderException, OperationalException, PricingError,
                                  TemporaryError)
from uptrading.uptradingbot import UptradingBot
from uptrading.persistence import Order, PairLocks, Trade
from uptrading.persistence.models import PairLock
from uptrading.plugins.protections.iprotection import ProtectionReturn
from uptrading.util.datetime_helpers import dt_now, dt_utc
from uptrading.worker import Worker
from tests.conftest import (EXMS, create_mock_trades, create_mock_trades_usdt,
                            get_patched_uptradingbot, get_patched_worker, log_has, log_has_re,
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    """
    mocker.patch('uptrading.rpc.telegram.Telegram', MagicMock())
    rpc_mock = mocker.patch('uptrading.uptradingbot.RPCManager.send_msg', MagicMock())
    return rpc_mock
...
def test_uptradingbot_state(mocker, default_conf_usdt, markets) -> None:
    mocker.patch(f'{EXMS}.markets', PropertyMock(return_value=markets))
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    assert uptrading.state is State.RUNNING

    default_conf_usdt.pop('initial_state')
    uptrading = UptradingBot(default_conf_usdt)
    assert uptrading.state is State.STOPPED


def test_process_stopped(mocker, default_conf_usdt) -> None:

    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    coo_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.cancel_all_open_orders')
    uptrading.process_stopped()
...
def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    uptrading.process()
    assert uptrading.rpc.process_msg_queue.call_count == 1


def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    mock_cleanup = mocker.patch('uptrading.uptradingbot.Trade.commit')
    coo_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.cancel_all_open_orders')
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    uptrading.cleanup()
    ...
    uptrading.config['cancel_open_orders_on_exit'] = True
    uptrading.cleanup()
    assert coo_mock.call_count == 1


def test_bot_cleanup_db_errors(mocker, default_conf_usdt, caplog) -> None:
    mocker.patch('uptrading.uptradingbot.Trade.commit',
                 side_effect=OperationalException())
    mocker.patch('uptrading.uptradingbot.UptradingBot.check_for_open_trades',
                 side_effect=OperationalException())
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    uptrading.emc = MagicMock()
    uptrading.emc.shutdown = MagicMock()
    uptrading.cleanup()
    assert uptrading.emc.shutdown.call_count == 1
...
def test_order_dict(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    uptrading = UptradingBot(conf)
    if runmode == RunMode.LIVE:
        assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
    assert uptrading.strategy.order_types['stoploss_on_exchange']
    ...
    uptrading = UptradingBot(conf)
    assert not uptrading.strategy.order_types['stoploss_on_exchange']
    assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
...
def test_get_trade_stake_amount(default_conf_usdt, mocker) -> None:
    ...
    result = uptrading.wallets.get_trade_stake_amount('ETH/USDT')
...
def test_load_strategy_no_keys(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    conf['runmode'] = runmode
    erm = mocker.patch('uptrading.uptradingbot.ExchangeResolver.load_exchange')

    uptrading = UptradingBot(conf)
    strategy_config = uptrading.strategy.config
...
def test_check_available_stake_amount(
    ...
    for i in range(0, max_open):
        ...
        if expected[i] is not None:
            ...
            uptrading.execute_entry('ETH/USDT', result)
        else:
            with pytest.raises(DependencyException):
                uptrading.wallets.get_trade_stake_amount('ETH/USDT')
...
def test_edge_called_in_process(mocker, edge_conf) -> None:
    ...
    patch_exchange(mocker)
    uptrading = UptradingBot(edge_conf)
    patch_get_signal(uptrading)
    uptrading.process()
    assert uptrading.active_pair_whitelist == ['NEO/BTC', 'LTC/BTC']
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert uptrading.wallets.get_trade_stake_amount(
        'NEO/BTC', uptrading.edge) == (999.9 * 0.5 * 0.01) / 0.20
    assert uptrading.wallets.get_trade_stake_amount(
        'LTC/BTC', uptrading.edge) == (999.9 * 0.5 * 0.01) / 0.21
...
def test_edge_overrides_stoploss(limit_order, fee, caplog, mocker,
    ...
    uptrading = UptradingBot(edge_conf)
    uptrading.active_pair_whitelist = ['NEO/BTC']
    patch_get_signal(uptrading)
    uptrading.strategy.min_roi_reached = MagicMock(return_value=False)
    uptrading.enter_positions()
    ...
    assert uptrading.handle_trade(trade) is not ignore_strat_sl
...
def test_create_trade(default_conf_usdt, ticker_usdt, limit_order,
    ...
    uptrading = UptradingBot(default_conf_usdt)
    patch_get_signal(uptrading, enter_short=is_short, enter_long=not is_short)
    uptrading.create_trade('ETH/USDT')
...
def test_create_trade_minimal_amount(
    ...
    uptrading = UptradingBot(default_conf_usdt)
    uptrading.config['stake_amount'] = stake_amount
    patch_get_signal(uptrading, enter_short=is_short, enter_long=not is_short)

    if create:
        assert uptrading.create_trade('ETH/USDT')
        if amount_enough:
        ...
        assert not uptrading.create_trade('ETH/USDT')
        if not max_open_trades:
            assert uptrading.wallets.get_trade_stake_amount('ETH/USDT', uptrading.edge) == 0
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    uptrading.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf"))
    create_mock_trades(fee, is_short)
    uptrading.handle_protections('ETC/BTC', '*')
    send_msg_mock = uptrading.rpc.send_msg
...
def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
    ...
    uptrading = UptradingBot(default_conf_usdt)
    patch_get_signal(uptrading, enter_long=False, exit_long=False)
...
def test_create_trades_preopen(default_conf_usdt, ticker_usdt, fee, mocker,
    ...
    uptrading.execute_entry('ETH/USDT', default_conf_usdt['stake_amount'])
    uptrading.execute_entry('NEO/BTC', default_conf_usdt['stake_amount'])
    ...
    assert uptrading.create_trade('ETH/USDT')
    assert uptrading.create_trade('NEO/BTC')
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.uptrading)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.uptrading.state == State.RUNNING

    worker._process_running()
    assert worker.uptrading.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(default_conf_usdt, ticker_usdt, limit_buy_order_usdt,
    ...
    assert pair not in uptrading.active_pair_whitelist
    uptrading.process()
    assert pair in uptrading.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(uptrading.active_pair_whitelist) == len(set(uptrading.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch.multiple(
        'uptrading.strategy.interface.IStrategy',
    ...
    uptrading = UptradingBot(default_conf_usdt)
    uptrading.strategy.informative_pairs = inf_pairs
    # patch_get_signal(uptrading)
...
def test_execute_entry(mocker, default_conf_usdt, fee, limit_order,
    ...
    mocker.patch('uptrading.exchange.gate.Gate.validate_ordertypes')
    patch_RPCManager(mocker)
    patch_exchange(mocker, id=exchange_name)
    uptrading = UptradingBot(default_conf_usdt)
    uptrading.strategy.confirm_trade_entry = MagicMock(return_value=False)
    uptrading.strategy.leverage = MagicMock(return_value=leverage)
    stake_amount = 2
    bid = 0.11
    ...
    mocker.patch.multiple(
        'uptrading.exchange.okx.Okx',
        get_max_pair_stake_amount=MagicMock(return_value=500000),
    )
    pair = 'ETH/USDT'

    assert not uptrading.execute_entry(pair, stake_amount, is_short=is_short)
    assert enter_rate_mock.call_count == 1
    assert enter_mm.call_count == 0
    assert uptrading.strategy.confirm_trade_entry.call_count == 1
    enter_rate_mock.reset_mock()

    open_order['id'] = '22'
    uptrading.strategy.confirm_trade_entry = MagicMock(return_value=True)
    assert uptrading.execute_entry(pair, stake_amount)
    assert enter_rate_mock.call_count == 1
    ...
    open_order['id'] = '33'
    fix_price = 0.06
    assert uptrading.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    assert uptrading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    uptrading.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert uptrading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order['id'] = '557'
    uptrading.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert uptrading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order['id'] = '66'
    mocker.patch(f'{EXMS}.create_order', MagicMock(return_value=order))
    assert not uptrading.execute_entry(pair, stake_amount)
    assert uptrading.strategy.leverage.call_count == 0 if trading_mode == 'spot' else 2
    ...
    with pytest.raises(PricingError, match="Could not determine entry price."):
        uptrading.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f'{EXMS}.get_rate', return_value=0.50)
    order['status'] = 'open'
    order['id'] = '5566'
    uptrading.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert uptrading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order['status'] = 'open'
    order['id'] = '5567'
    uptrading.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order['status'] = 'open'
    order['id'] = '5568'
    uptrading.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert uptrading.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    uptrading.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)

    assert uptrading.execute_entry(pair, 2000, is_short=is_short)
    ...
    uptrading.strategy.leverage.reset_mock()
    assert uptrading.execute_entry(pair, 200, leverage_=3)
    assert uptrading.strategy.leverage.call_count == 0
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    uptrading.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert uptrading.execute_entry(pair, stake_amount)

    limit_order[entry_side(is_short)]['id'] = '222'
    uptrading.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert uptrading.execute_entry(pair, stake_amount)
...
def test_execute_entry_min_leverage(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    stake_amount = 2
    pair = 'SOL/BUSD:BUSD'
    uptrading.strategy.leverage = MagicMock(return_value=5.0)
...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    order = limit_order[entry_side(is_short)]
    mocker.patch('uptrading.uptradingbot.UptradingBot.handle_trade', MagicMock(return_value=True))
    ...
    uptrading = UptradingBot(default_conf_usdt)
    uptrading.strategy.order_types['stoploss_on_exchange'] = True
    ...
    uptrading.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(mocker, default_conf_usdt, fee, caplog, is_short,
    ...
    assert uptrading.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    assert trade.stoploss_order_id == "13434334"
    ...
    assert uptrading.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    uptrading.handle_stoploss_on_exchange(trade)
    ...
    uptrading.config['trailing_stop'] = True
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # Uptrading cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(return_value={
        'id': "101",
        'status': 'open',
        'type': 'stop_loss_limit',
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    uptrading.create_stoploss_order(trade, 200)
    assert trade.stoploss_order_id is None
...
def test_create_stoploss_order_insufficient_funds(
    ...
    mock_insuf = mocker.patch('uptrading.uptradingbot.UptradingBot.handle_insufficient_funds')
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=MagicMock(return_value={
            'bid': 1.9,
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    uptrading.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    uptrading.strategy.order_types['stoploss_on_exchange_interval'] = 60
    ...
    assert uptrading.handle_trade(trade) is False
    assert uptrading.handle_stoploss_on_exchange(trade) is False
    ...
    uptrading.strategy.order_types['stoploss_on_exchange_interval'] = 0
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair='ETH/USDT',
        order_types=uptrading.strategy.order_types,
    ...
    assert uptrading.handle_trade(trade) is True
    assert trade.stoploss_order_id is None
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    uptrading.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    uptrading.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    uptrading.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    uptrading.strategy.stoploss = -0.02
    ...
    uptrading.active_pair_whitelist = uptrading.edge.adjust(uptrading.active_pair_whitelist)
...
def test_enter_positions(mocker, default_conf_usdt, return_value, side_effect,
    ...
    mock_ct = mocker.patch(
        'uptrading.uptradingbot.UptradingBot.create_trade',
        MagicMock(
...
def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    Trade.commit()
    trades = [trade]
    uptrading.wallets.update()
    n = uptrading.exit_positions(trades)
    assert n == 0
    # Test amount not modified by fee-logic
    assert not log_has_re(r'Applying fee to amount for Trade .*', caplog)

    gra = mocker.patch('uptrading.uptradingbot.UptradingBot.get_real_amount', return_value=0.0)
...
def test_exit_positions_exception(mocker, default_conf_usdt, limit_order, caplog, is_short) -> None:
    ...
    mocker.patch(
        'uptrading.uptradingbot.UptradingBot.handle_trade',
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch(f'{EXMS}.fetch_order', return_value=order)
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=[])
    mocker.patch('uptrading.uptradingbot.UptradingBot.get_real_amount', return_value=0.0)
    order_id = order['id']
    ...
    mocker.patch('uptrading.uptradingbot.UptradingBot.get_real_amount', return_value=0.01)
    assert trade.amount == 30.0
    # test amount modified by fee-logic
    uptrading.update_trade_state(trade, order_id)
    ...
    mocker.patch('uptrading.uptradingbot.UptradingBot.get_real_amount', side_effect=ValueError)
    mocker.patch(f'{EXMS}.fetch_order', return_value=limit_buy_order_usdt_new)
    res = uptrading.update_trade_state(trade, order_id)
    # Cancelled empty
    assert res is True
...
def test_update_trade_state_withorderdict(
    ...
    uptrading.update_trade_state(trade, order_id, order)
...
def test_update_trade_state_exception(mocker, default_conf_usdt, is_short, limit_order,
    ...
    mocker.patch(
        'uptrading.uptradingbot.UptradingBot.get_real_amount',
        side_effect=DependencyException()
    )
    uptrading.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("uptrading.uptradingbot.UptradingBot.get_real_amount", MagicMock())
    uptrading.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_sell(
    ...
    mocker.patch('uptrading.wallets.Wallets.update', wallet_mock)
    ...
    uptrading.update_trade_state(trade, trade.open_orders_ids[-1], l_order)
...
def test_handle_trade(
    ...
    patch_get_signal(uptrading, enter_long=False, exit_short=is_short,
...
def test_handle_overlapping_signals(
    ...
    uptrading = UptradingBot(default_conf_usdt)
    if is_short:
        patch_get_signal(uptrading, enter_long=False, enter_short=True, exit_short=True)
    else:
        patch_get_signal(uptrading, enter_long=True, exit_long=True)
    uptrading.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(uptrading, enter_long=False)
    assert uptrading.handle_trade(trades[0]) is False
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    nb_trades = len(trades)
    ...
    if is_short:
        patch_get_signal(uptrading, enter_long=False, exit_short=True)
    else:
        patch_get_signal(uptrading, enter_long=False, exit_long=True)
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    assert uptrading.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(default_conf_usdt, ticker_usdt, limit_order_open, fee, mocker, caplog,
    ...
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    patch_get_signal(uptrading, enter_short=is_short, enter_long=not is_short)
    uptrading.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    caplog.clear()
    patch_get_signal(uptrading)
    assert uptrading.handle_trade(trade)
...
def test_handle_trade_use_exit_signal(
    ...
    patch_get_signal(uptrading, enter_long=False, exit_long=False)
    assert not uptrading.handle_trade(trade)
...
def test_close_trade(
    ...
    with pytest.raises(DependencyException, match=r'.*closed trade.*'):
        uptrading.handle_trade(trade)


def test_bot_loop_start_called_once(mocker, default_conf_usdt, caplog):
    ftbot = get_patched_uptradingbot(mocker, default_conf_usdt)
    mocker.patch('uptrading.uptradingbot.UptradingBot.create_trade')
...
def test_manage_open_orders_entry_usercustom(
    ...
    uptrading.manage_open_orders()
    assert cancel_order_mock.call_count == 0

    # Return false - trade remains open
    uptrading.strategy.check_entry_timeout = MagicMock(return_value=False)
    uptrading.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    ...
    nb_trades = len(trades)
    assert nb_trades == 1
    assert uptrading.strategy.check_entry_timeout.call_count == 1
    uptrading.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    assert nb_trades == 1
    assert uptrading.strategy.check_entry_timeout.call_count == 1
    uptrading.strategy.check_entry_timeout = MagicMock(return_value=True)
...
def test_manage_open_orders_entry(
    ...
    uptrading.strategy.check_entry_timeout = MagicMock(return_value=False)
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=1234)
    ...
    assert uptrading.strategy.check_entry_timeout.call_count == 0
    # Entry adjustment is never called
    assert uptrading.strategy.adjust_entry_price.call_count == 0
...
def test_adjust_entry_cancel(
    ...
    uptrading.strategy.ft_check_timed_out = MagicMock(return_value=False)

    # check that order is cancelled
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=None)
    uptrading.manage_open_orders()
    trades = Trade.session.scalars(
        select(Trade)
        .where(Order.ft_trade_id == Trade.id)
    ...
    assert uptrading.strategy.adjust_entry_price.call_count == 1
...
def test_adjust_entry_replace_fail(
    ...
    mocker.patch('uptrading.uptradingbot.sleep')
    ...
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=12234)
    uptrading.manage_open_orders()
    trades = Trade.session.scalars(
        select(Trade)
        .where(Order.ft_trade_id == Trade.id)
...
def test_adjust_entry_replace_fail_create_order(
    ...
    mocker.patch('uptrading.uptradingbot.sleep')
    mocker.patch('uptrading.uptradingbot.UptradingBot.execute_entry',
                 side_effect=DependencyException())
...
def test_adjust_entry_maintain_replace(
    ...
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=old_order['price'])
    uptrading.manage_open_orders()
    trades = Trade.session.scalars(
        select(Trade)
        .where(Order.ft_is_open.is_(True))
    ...
    uptrading.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
def test_manage_open_orders_exit_usercustom(
    ...
    et_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.execute_trade_exit')
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
    ...
    uptrading.strategy.check_exit_timeout = MagicMock(return_value=False)
    uptrading.strategy.check_entry_timeout = MagicMock(return_value=False)
    # Return false - No impact
    uptrading.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 1
    assert uptrading.strategy.check_exit_timeout.call_count == 1
    assert uptrading.strategy.check_entry_timeout.call_count == 0

    uptrading.strategy.check_exit_timeout = MagicMock(side_effect=KeyError)
    uptrading.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    uptrading.strategy.check_exit_timeout = MagicMock(return_value=True)
    uptrading.strategy.check_entry_timeout = MagicMock(return_value=True)
    uptrading.manage_open_orders()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 2
    ...
    mocker.patch('uptrading.persistence.Trade.get_canceled_exit_order_count', return_value=1)
    mocker.patch('uptrading.uptradingbot.UptradingBot.execute_trade_exit',
                 side_effect=DependencyException)
    uptrading.manage_open_orders()
    ...
    with patch('uptrading.uptradingbot.UptradingBot.handle_cancel_exit', return_value=False):
        uptrading.manage_open_orders()
...
def test_manage_open_orders_exit(
    ...
    assert uptrading.strategy.check_exit_timeout.call_count == 0
    assert uptrading.strategy.check_entry_timeout.call_count == 0
...
def test_manage_open_orders_partial_fee(
    ...
    mocker.patch('uptrading.wallets.Wallets.get_free', MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
def test_manage_open_orders_partial_except(
    ...
    mocker.patch('uptrading.uptradingbot.UptradingBot.get_real_amount',
...
def test_manage_open_orders_exception(default_conf_usdt, ticker_usdt, open_trade_usdt, mocker,
    ...
    mocker.patch.multiple(
        'uptrading.uptradingbot.UptradingBot',
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    uptrading = UptradingBot(default_conf_usdt)
    uptrading._notify_enter_cancel = MagicMock()
    ...
    assert uptrading.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order['filled'] = 0.01
    assert not uptrading.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not uptrading.handle_cancel_enter(
...
def test_handle_cancel_enter_exchanges(mocker, caplog, default_conf_usdt, is_short, fee,
    ...
    notify_mock = mocker.patch('uptrading.uptradingbot.UptradingBot._notify_enter_cancel')
    uptrading = UptradingBot(default_conf_usdt)
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert uptrading.handle_cancel_enter(
...
def test_handle_cancel_exit_limit(mocker, default_conf_usdt, fee, is_short,
    ...
    mocker.patch('uptrading.uptradingbot.UptradingBot.handle_order_fee')
    ...
    assert uptrading.handle_cancel_exit(trade, order, order_obj, reason)
    assert cancel_order_mock.call_count == 1
    ...
    assert not uptrading.handle_cancel_exit(trade, order, order_obj, reason)
...
def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    ...
    # assert not uptrading.handle_cancel_exit(trade, order, reason)
...
def test_execute_trade_exit_up(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_up, mocker,
    ...
    uptrading = UptradingBot(default_conf_usdt)
    patch_get_signal(uptrading, enter_short=is_short, enter_long=not is_short)
    uptrading.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    assert trade.is_short == is_short
    assert trade
    assert uptrading.strategy.confirm_trade_exit.call_count == 0
    ...
    assert uptrading.strategy.confirm_trade_exit.call_count == 1
    assert id(uptrading.strategy.confirm_trade_exit.call_args_list[0][1]['trade']) != id(trade)
    assert uptrading.strategy.confirm_trade_exit.call_args_list[0][1]['trade'].id == trade.id

    # Repatch with true
    uptrading.strategy.confirm_trade_exit = MagicMock(return_value=True)
    uptrading.execute_trade_exit(
        trade=trade,
        limit=(ticker_usdt_sell_down()['ask'] if is_short else ticker_usdt_sell_up()['bid']),
        exit_check=ExitCheckTuple(exit_type=ExitType.ROI)
...
def test_execute_trade_exit_custom_exit_price(
    ...
    uptrading.strategy.custom_exit_price = lambda **kwargs: 2.25
    uptrading.execute_trade_exit(
        trade=trade,
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    mocker.patch('uptrading.wallets.Wallets.get_free', MagicMock(return_value=300))
    create_order_mock = MagicMock(side_effect=[
        {'id': '12345554'},
        {'id': '12345555'},
    ])
    ...
    uptrading.config['dry_run'] = False
    trade.stoploss_order_id = "abcd"

    uptrading.execute_trade_exit(trade=trade, limit=1234,
                                 exit_check=ExitCheckTuple(exit_type=ExitType.STOP_LOSS))
    assert create_order_mock.call_count == 2
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    uptrading = UptradingBot(default_conf_usdt)
    uptrading.strategy.order_types['stoploss_on_exchange'] = True
    patch_get_signal(uptrading, enter_long=not is_short, enter_short=is_short)
...
def test_execute_trade_exit_market_order(
    ...
    uptrading.config['order_types']['exit'] = 'market'
...
def test_execute_trade_exit_insufficient_funds_error(default_conf_usdt, ticker_usdt, fee, is_short,
    ...
    assert not uptrading.execute_trade_exit(
        trade=trade,
        limit=ticker_usdt_sell_up()['ask' if is_short else 'bid'],
...
def test_exit_profit_only(
    ...
    uptrading = UptradingBot(default_conf_usdt)
    patch_get_signal(uptrading, enter_short=is_short, enter_long=not is_short)
    uptrading.strategy.custom_exit = MagicMock(return_value=None)
    if exit_type == ExitType.EXIT_SIGNAL.value:
        uptrading.strategy.min_roi_reached = MagicMock(return_value=False)
    else:
        uptrading.strategy.ft_stoploss_reached = MagicMock(return_value=ExitCheckTuple(
    ...
    if profit_only:
        ...
        uptrading.strategy.custom_exit.call_count == 1

    patch_get_signal(uptrading, enter_long=False, exit_short=is_short, exit_long=not is_short)
    assert uptrading.handle_trade(trade) is handle_first

    if handle_second:
        uptrading.strategy.exit_profit_offset = 0.0
        assert uptrading.handle_trade(trade) is True
...
def test_sell_not_enough_balance(default_conf_usdt, limit_order, limit_order_open,
    ...
    trade.update_trade(oobj)
    patch_get_signal(uptrading, enter_long=False, exit_long=True)
    mocker.patch('uptrading.wallets.Wallets.get_free', MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch('uptrading.wallets.Wallets.get_free', MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch('uptrading.wallets.Wallets.update')
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to exit trade."):
            assert uptrading._safe_exit_amount(trade, trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert trade.amount != amount_wallet
        assert uptrading._safe_exit_amount(trade, trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r'.*Falling back to wallet-amount.', caplog)
        assert trade.amount == amount_wallet
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert uptrading._safe_exit_amount(trade, trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(default_conf_usdt, ticker_usdt, fee,
    ...
    assert uptrading.strategy.is_pair_locked(trade.pair, side='*')
    # Boths sides are locked
    assert uptrading.strategy.is_pair_locked(trade.pair, side='long')
    assert uptrading.strategy.is_pair_locked(trade.pair, side='short')
...
def test_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open, is_short,
    ...
    if is_short:
        patch_get_signal(uptrading, enter_long=False, exit_short=False, exit_tag='something')
    else:
        patch_get_signal(uptrading, enter_long=False, exit_long=False, exit_tag='something')
    assert uptrading.handle_trade(trade) is True
...
def test_disable_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open,
    ...
    patch_get_signal(uptrading, enter_long=not is_short, enter_short=is_short, exit_short=is_short)
    assert uptrading.handle_trade(trade) is True
...
def test_get_real_amount_quote(default_conf_usdt, trades_for_order, buy_order_fee, fee, caplog,
    ...
    assert uptrading.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
def test_get_real_amount_quote_dust(default_conf_usdt, trades_for_order, buy_order_fee, fee,
                                    caplog, mocker):
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=trades_for_order)
    walletmock = mocker.patch('uptrading.wallets.Wallets.update')
    mocker.patch('uptrading.wallets.Wallets.get_free', return_value=8.1122)
    amount = sum(x['amount'] for x in trades_for_order)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    assert uptrading.get_real_amount(trade, buy_order_fee, order_obj) is None
...
def test_get_real_amount(
    ...
    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = uptrading.get_real_amount(trade, buy_order, order_obj)
...
def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    assert uptrading.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            'Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
...
def test_get_real_amount_invalid_order(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    assert uptrading.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
def test_get_real_amount_fees_order(default_conf_usdt, market_buy_order_usdt_doublefee,
    ...
    assert uptrading.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
...
def test_get_real_amount_wrong_amount(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        uptrading.get_real_amount(trade, limit_buy_order_usdt, order_obj)
...
def test_get_real_amount_wrong_amount_rounding(default_conf_usdt, trades_for_order, buy_order_fee,
    ...
    assert pytest.approx(uptrading.get_real_amount(
...
def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert uptrading.get_real_amount(trade, order, order_obj) is None
...
def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = uptrading.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
    ...
    caplog.clear()
    uptrading.config['exchange']['unknown_fee_rate'] = 1
    res = uptrading.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
def test_apply_fee_conditional(default_conf_usdt, fee, mocker, caplog,
    ...
    walletmock = mocker.patch('uptrading.wallets.Wallets.update')
    mocker.patch('uptrading.wallets.Wallets.get_free', return_value=wallet)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert uptrading.apply_fee_conditional(trade, 'LTC', amount, fee_abs, order) == amount_exp
    assert walletmock.call_count == 1
...
def test_apply_fee_conditional_multibuy(default_conf_usdt, fee, mocker, caplog,
    ...
    assert uptrading.apply_fee_conditional(trade, 'LTC', amount, fee_abs, order1) == amount_exp
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
def test_order_book_entry_pricing1(mocker, default_conf_usdt, order_book_l2, exception_thrown,
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            uptrading.exchange.get_rate('ETH/USDT', side="entry", is_short=False, refresh=True)
        assert log_has_re(
            r'ETH/USDT - Entry Price at location 1 from orderbook could not be determined.', caplog)
    else:
        assert uptrading.exchange.get_rate(
...
def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert uptrading._check_depth_of_market('ETH/BTC', conf, side=SignalDirection.LONG) is False
...
def test_startup_state(default_conf_usdt, mocker):
    ...
    assert worker.uptrading.state is State.RUNNING
...
def test_startup_trade_reinit(default_conf_usdt, edge_conf, mocker):
    ...
    mocker.patch('uptrading.persistence.Trade.stoploss_reinitialization', reinit_mock)
    ...
    ftbot = get_patched_uptradingbot(mocker, edge_conf)
    ftbot.startup()
...
def test_sync_wallet_dry_run(mocker, default_conf_usdt, ticker_usdt, fee, limit_buy_order_usdt_open,
    ...
    bot = get_patched_uptradingbot(mocker, default_conf_usdt)
...
def test_cancel_all_open_orders(mocker, default_conf_usdt, fee, limit_order, limit_order_open,
    ...
    buy_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.handle_cancel_enter')
    sell_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.handle_cancel_exit')

    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    create_mock_trades(fee, is_short=is_short)
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == MOCK_TRADE_COUNT
    uptrading.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    ...
    uptrading.check_for_open_trades()
    assert uptrading.rpc.send_msg.call_count == 0
    ...
    uptrading.check_for_open_trades()
    assert uptrading.rpc.send_msg.call_count == 1
    assert 'Handle these trades manually' in uptrading.rpc.send_msg.call_args[0][0]['status']
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    uptrading.startup_update_open_orders()
    ...
    hto_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.handle_cancel_order')
...
def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    ...
    uptrading.startup_backpopulate_precision()
    trades = Trade.get_trades().all()
    for trade in trades:
...
def test_update_trades_without_assigned_fees(mocker, default_conf_usdt, fee, is_short):
    ...
    uptrading.update_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    mock_uts = mocker.patch('uptrading.uptradingbot.UptradingBot.update_trade_state')

    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order', return_value={'status': 'open'})
    create_mock_trades(fee, is_short)
    trades = Trade.get_trades().all()

    uptrading.handle_insufficient_funds(trades[3])
    ...
    uptrading.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    uptrading.handle_insufficient_funds(trades[4])
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    ...
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    mock_uts = mocker.spy(uptrading, 'update_trade_state')
    ...
    Trade.session.add(trade)
    uptrading.handle_onexchange_order(trade)
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    uptrading = UptradingBot(default_conf_usdt)
    uptrading.config['custom_price_max_distance_ratio'] = 0.02
    ...
    valid_price_from_string = uptrading.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = uptrading.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = uptrading.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = uptrading.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = uptrading.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = uptrading.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_update_funding_fees_schedule(mocker, default_conf, trading_mode, calls, time_machine,
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch('uptrading.uptradingbot.UptradingBot.update_funding_fees', return_value=True)
    default_conf['trading_mode'] = trading_mode
    default_conf['margin_mode'] = 'isolated'
    uptrading = get_patched_uptradingbot(mocker, default_conf)

    time_machine.move_to(f"{t2} +00:00", tick=False)
    # Check schedule jobs in debugging with uptrading._schedule.jobs
    uptrading._schedule.run_pending()

    assert uptrading.update_funding_fees.call_count == calls
...
def test_update_funding_fees(
    ...
    uptrading.execute_entry('ETH/USDT', 123, is_short=is_short)
    uptrading.execute_entry('LTC/USDT', 2.0, is_short=is_short)
    uptrading.execute_entry('XRP/USDT', 123, is_short=is_short)
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    assert uptrading.execute_entry(pair, stake_amount, trade=trade)
    ...
    assert uptrading.execute_trade_exit(trade=trade, limit=8,
...
def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 MagicMock(return_value=closed_successful_buy_order))
    assert uptrading.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 MagicMock(return_value=closed_sell_dca_order_1))
    assert uptrading.execute_trade_exit(trade=trade, limit=ask,
                                        exit_check=ExitCheckTuple(exit_type=ExitType.PARTIAL_EXIT),
...
def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    ...
    for idx, (order, result) in enumerate(data):
        ...
        if order[0] == 'buy':
            assert uptrading.execute_entry(pair, amount, trade=trade)
        else:
            assert uptrading.execute_trade_exit(
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    mocker.patch('uptrading.uptradingbot.UptradingBot.check_and_call_adjust_trade_position',
                 side_effect=DependencyException())

    create_mock_trades(fee)

    uptrading.process_open_trade_positions()
...
def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    create_mock_trades(fee)
    caplog.set_level(logging.DEBUG)
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=10)
    uptrading.process_open_trade_positions()
