...
from uptrading.constants import DATETIME_PRINT_FORMAT
from uptrading.data.converter import ohlcv_to_dataframe
from uptrading.data.history.history_utils import (_download_pair_history, _download_trades_history,
                                                  _load_cached_data_for_updating, get_timerange,
                                                  load_data, load_pair_history,
                                                  refresh_backtest_ohlcv_data,
                                                  refresh_backtest_trades_data, refresh_data,
                                                  validate_backtest_data)
from uptrading.data.history.idatahandler import get_datahandler
from uptrading.data.history.jsondatahandler import JsonDataHandler, JsonGzDataHandler
from uptrading.enums import CandleType
from uptrading.exchange import timeframe_to_minutes
from uptrading.misc import file_dump_json
from uptrading.resolvers import StrategyResolver
from uptrading.util import dt_ts, dt_utc
...
def test_load_data_7min_timeframe(caplog, testdatadir) -> None:
    ...
    assert log_has(
        ...
        'Use `uptrading download-data` to download the data', caplog
    )
...
def test_load_data_startup_candles(mocker, testdatadir) -> None:
    ltfmock = mocker.patch(
        'uptrading.data.history.featherdatahandler.FeatherDataHandler._ohlcv_load',
...
@pytest.mark.parametrize("pair,timeframe,expected_result,candle_type", [
    ("ETH/BTC", "5m", "uptrading/hello/world/ETH_BTC-5m.json", ""),
    ("ETH/USDT", "1M", "uptrading/hello/world/ETH_USDT-1Mo.json", ""),
    ("Fabric Token/ETH", "5m", "uptrading/hello/world/Fabric_Token_ETH-5m.json", ""),
    ("ETHH20", "5m", "uptrading/hello/world/ETHH20-5m.json", ""),
    (".XBTBON2H", "5m", "uptrading/hello/world/_XBTBON2H-5m.json", ""),
    ("ETHUSD.d", "5m", "uptrading/hello/world/ETHUSD_d-5m.json", ""),
    ("ACC_OLD/BTC", "5m", "uptrading/hello/world/ACC_OLD_BTC-5m.json", ""),
    ("ETH/BTC", "5m", "uptrading/hello/world/futures/ETH_BTC-5m-mark.json", "mark"),
    ("ACC_OLD/BTC", "5m", "uptrading/hello/world/futures/ACC_OLD_BTC-5m-index.json", "index"),
])
def test_json_pair_data_filename(pair, timeframe, expected_result, candle_type):
    fn = JsonDataHandler._pair_data_filename(
        Path('uptrading/hello/world'),
        pair,
        timeframe,
...
@pytest.mark.parametrize("pair,expected_result", [
    ("ETH/BTC", 'uptrading/hello/world/ETH_BTC-trades.json'),
    ("Fabric Token/ETH", 'uptrading/hello/world/Fabric_Token_ETH-trades.json'),
    ("ETHH20", 'uptrading/hello/world/ETHH20-trades.json'),
    (".XBTBON2H", 'uptrading/hello/world/_XBTBON2H-trades.json'),
    ("ETHUSD.d", 'uptrading/hello/world/ETHUSD_d-trades.json'),
    ("ACC_OLD_BTC", 'uptrading/hello/world/ACC_OLD_BTC-trades.json'),
])
def test_json_pair_trades_filename(pair, expected_result):
    fn = JsonDataHandler._pair_trades_filename(Path('uptrading/hello/world'), pair)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)

    fn = JsonGzDataHandler._pair_trades_filename(Path('uptrading/hello/world'), pair)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result + '.gz')
...
def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    ...
    json_dump_mock = mocker.patch(
        'uptrading.data.history.featherdatahandler.FeatherDataHandler.ohlcv_store',
        return_value=None)
...
def test_refresh_backtest_ohlcv_data(
    ...
    dl_mock = mocker.patch('uptrading.data.history.history_utils._download_pair_history')
...
def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch('uptrading.data.history.history_utils._download_trades_history',
                           MagicMock())
    mocker.patch(f'{EXMS}.markets', PropertyMock(return_value=markets))
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch.object(Path, "unlink", MagicMock())
