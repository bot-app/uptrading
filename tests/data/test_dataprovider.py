...
def test_historic_ohlcv(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("uptrading.data.dataprovider.load_pair_history", historymock)
...
def test_historic_ohlcv_dataformat(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("uptrading.data.history.hdf5datahandler.HDF5DataHandler._ohlcv_load", hdf5loadmock)
    mocker.patch("uptrading.data.history.featherdatahandler.FeatherDataHandler._ohlcv_load",
                 featherloadmock)
...
def test_emit_df(mocker, default_conf, ohlcv_history):
    mocker.patch('uptrading.rpc.rpc_manager.RPCManager.__init__', MagicMock())
    rpc_mock = mocker.patch('uptrading.rpc.rpc_manager.RPCManager', MagicMock())
    send_mock = mocker.patch('uptrading.rpc.rpc_manager.RPCManager.send_msg', MagicMock())
