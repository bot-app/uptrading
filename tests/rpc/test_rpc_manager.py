...
from uptrading.rpc.api_server.webserver import ApiServer
from tests.conftest import get_patched_uptradingbot, log_has


def test__init__(mocker, default_conf) -> None:
    default_conf['telegram']['enabled'] = False

    rpc_manager = RPCManager(get_patched_uptradingbot(mocker, default_conf))
...
def test_cleanup_telegram_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    telegram_mock = mocker.patch('uptrading.rpc.telegram.Telegram.cleanup', MagicMock())
    default_conf['telegram']['enabled'] = False

    uptradingbot = get_patched_uptradingbot(mocker, default_conf)
    rpc_manager = RPCManager(uptradingbot)
    rpc_manager.cleanup()
...
def test_send_msg_telegram_disabled(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch('uptrading.rpc.telegram.Telegram.send_msg', MagicMock())
    default_conf['telegram']['enabled'] = False
...
def test_send_msg_telegram_error(mocker, default_conf, caplog) -> None:
    mocker.patch('uptrading.rpc.telegram.Telegram._init', MagicMock())
    mocker.patch('uptrading.rpc.telegram.Telegram.send_msg', side_effect=ValueError())
    default_conf['telegram']['enabled'] = True
    uptradingbot = get_patched_uptradingbot(mocker, default_conf)
    rpc_manager = RPCManager(uptradingbot)
    rpc_manager.send_msg({
...
def test_process_msg_queue(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch('uptrading.rpc.telegram.Telegram.send_msg')
    default_conf['telegram']['enabled'] = True
...
def test_send_msg_webhook_CustomMessagetype(mocker, default_conf, caplog) -> None:
    ...
    default_conf['telegram']['enabled'] = False
    default_conf['webhook'] = {'enabled': True, 'url': "https://DEADBEEF.com"}
    mocker.patch('uptrading.rpc.webhook.Webhook.send_msg',
...
def test_startupmessages_telegram_enabled(mocker, default_conf) -> None:
    ...
    uptradingbot = get_patched_uptradingbot(mocker, default_conf)
    rpc_manager = RPCManager(uptradingbot)
    rpc_manager.startup_messages(default_conf, uptradingbot.pairlists, uptradingbot.protections)
...
def test_init_apiserver_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    run_mock = MagicMock()
    mocker.patch('uptrading.rpc.api_server.ApiServer.start_api', run_mock)
    default_conf['telegram']['enabled'] = False
    rpc_manager = RPCManager(get_patched_uptradingbot(mocker, default_conf))
