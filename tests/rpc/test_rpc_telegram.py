...
from uptrading.constants import CANCEL_REASON
from uptrading.edge import PairInfo
from uptrading.enums import (ExitType, MarketDirection, RPCMessageType, RunMode, SignalDirection,
                             State)
from uptrading.exceptions import OperationalException
from uptrading.uptradingbot import UptradingBot
from uptrading.loggers import setup_logging
from uptrading.persistence import PairLocks, Trade
from uptrading.persistence.models import Order
from uptrading.rpc import RPC
from uptrading.rpc.rpc import RPCException
from uptrading.rpc.telegram import Telegram, authorized_only
from uptrading.util.datetime_helpers import dt_now
from tests.conftest import (CURRENT_TEST_STRATEGY, EXMS, create_mock_trades,
                            create_mock_trades_usdt, get_patched_uptradingbot, log_has, log_has_re,
                            patch_exchange, patch_get_signal, patch_whitelist)


@pytest.fixture(autouse=True)
def mock_exchange_loop(mocker):
    mocker.patch('uptrading.exchange.exchange.Exchange._init_async_loop')
...
def get_telegram_testobject(mocker, default_conf, mock=True, ftbot=None):
    ...
    if mock:
        mocker.patch.multiple(
            'uptrading.rpc.telegram.Telegram',
            _init=MagicMock(),
            _send_msg=msg_mock,
    ...
    if not ftbot:
        mocker.patch('uptrading.exchange.exchange.Exchange._init_async_loop')
        ftbot = get_patched_uptradingbot(mocker, default_conf)
    rpc = RPC(ftbot)
...
def test_telegram__init__(default_conf, mocker) -> None:
    mocker.patch('uptrading.rpc.telegram.Telegram._init', MagicMock())
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    app_mock = MagicMock()
    mocker.patch('uptrading.rpc.telegram.Telegram._start_thread', MagicMock())
    mocker.patch('uptrading.rpc.telegram.Telegram._init_telegram_app', return_value=app_mock)
    mocker.patch('uptrading.rpc.telegram.Telegram._startup_telegram', AsyncMock())
...
async def test_telegram_startup(default_conf, mocker) -> None:
    ...
    sleep_mock = mocker.patch('uptrading.rpc.telegram.asyncio.sleep', AsyncMock())
...
async def test_telegram_cleanup(default_conf, mocker, ) -> None:
    ...
    # mocker.patch('uptrading.rpc.telegram.Application', app_mock)
...
async def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch('uptrading.rpc.telegram.Telegram._status_table', status_table)

    mocker.patch.multiple(
        'uptrading.rpc.rpc.RPC',
...
async def test_status_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    telegram, uptradingbot, msg_mock = get_telegram_testobject(mocker, default_conf)

    patch_get_signal(uptradingbot)

    uptradingbot.state = State.STOPPED
    ...
    uptradingbot.state = State.RUNNING
    await telegram._status(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert 'no active trade' in msg_mock.call_args_list[0][0][0]
    msg_mock.reset_mock()
    ...
    mocker.patch('uptrading.rpc.telegram.MAX_MESSAGE_LENGTH', 500)
...
async def test_daily_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    mocker.patch(
        'uptrading.rpc.rpc.CryptoToFiatConverter._find_price',
        return_value=1.1
    )
    mocker.patch.multiple(
        EXMS,
    ...
    telegram, uptradingbot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
async def test_telegram_profit_handle(
    ...
    mocker.patch('uptrading.rpc.rpc.CryptoToFiatConverter._find_price', return_value=1.1)
    mocker.patch.multiple(
        EXMS,
    ...
    mocker.patch('uptrading.wallets.Wallets.get_starting_balance', return_value=1000)
...
async def test_telegram_stats(default_conf, update, ticker, fee, mocker, is_short) -> None:
    mocker.patch('uptrading.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker,
        get_fee=fee,
...
async def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    ...
    uptradingbot.config['dry_run'] = False
    await telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
async def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch('uptrading.rpc.rpc.RPC._rpc_balance', return_value={
        'currencies': balances,
        'total': 100.0,
        'total_bot': 100.0,
        'symbol': 100.0,
...
async def test_start_handle(default_conf, update, mocker) -> None:
    ...
    uptradingbot.state = State.STOPPED
    assert uptradingbot.state == State.STOPPED
    await telegram._start(update=update, context=MagicMock())
    assert uptradingbot.state == State.RUNNING
    assert msg_mock.call_count == 1
...
async def test_stopbuy_handle(default_conf, update, mocker) -> None:
    ...
    assert uptradingbot.config['max_open_trades'] != 0
...
async def test_reload_config_handle(default_conf, update, mocker) -> None:
    ...
    assert uptradingbot.state == State.RELOAD_CONFIG
    assert msg_mock.call_count == 1
...
async def test_telegram_forceexit_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('uptrading.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    msg_mock = mocker.patch('uptrading.rpc.telegram.Telegram.send_msg', MagicMock())
    mocker.patch('uptrading.rpc.telegram.Telegram._init', MagicMock())
    patch_exchange(mocker)
    ...
    uptradingbot = UptradingBot(default_conf)
    rpc = RPC(uptradingbot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(uptradingbot)
...
async def test_telegram_force_exit_down_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('uptrading.rpc.fiat_convert.CryptoToFiatConverter._find_price',
...
async def test_force_exit_no_pair(default_conf, update, ticker, fee, mocker) -> None:
    ...
    femock = mocker.patch('uptrading.rpc.rpc.RPC._rpc_force_exit')
    telegram, uptradingbot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
async def test_force_enter_handle(default_conf, update, mocker) -> None:
    ...
    mocker.patch('uptrading.rpc.rpc.RPC._rpc_force_entry', fbuy_mock)

    telegram, uptradingbot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(uptradingbot)
...
async def test_telegram_entry_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch('uptrading.rpc.rpc.RPC._rpc_enter_tag_performance',
...
async def test_telegram_exit_reason_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch('uptrading.rpc.rpc.RPC._rpc_exit_reason_performance',
                 side_effect=RPCException('Error'))
...
async def test_telegram_mix_tag_performance_handle(default_conf_usdt, update, ticker, fee,
    ...
    msg_mock.reset_mock()
    mocker.patch('uptrading.rpc.rpc.RPC._rpc_mix_tag_performance',
                 side_effect=RPCException('Error'))
...
async def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert ("Blacklist contains 3 pairs\n`DOGE/BTC, HOT/BTC, ETH/BTC`"
            in msg_mock.call_args_list[0][0][0])
    assert uptradingbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert ("Blacklist contains 4 pairs\n`DOGE/BTC, HOT/BTC, ETH/BTC, XRP/.*`"
            in msg_mock.call_args_list[0][0][0])
    assert uptradingbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
async def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "uptrading\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch('uptrading.rpc.telegram.MAX_MESSAGE_LENGTH', 200)
    context = MagicMock()
    context.args = []
    await telegram._logs(update=update, context=context)
...
async def test_edge_enabled(edge_conf, update, mocker) -> None:
    mocker.patch('uptrading.edge.Edge._cached_pairs', mocker.PropertyMock(
        return_value={
...
async def test_version_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    uptradingbot.strategy.version = lambda: '1.1.1'
...
async def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    uptradingbot.config['trailing_stop'] = True
    await telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert '*Mode:* `{}`'.format('Dry-run') in msg_mock.call_args_list[0][0][0]
    assert '*Exchange:* `binance`' in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_enter_notification(default_conf, mocker, caplog, message_type,
    ...
    mocker.patch('uptrading.data.dataprovider.DataProvider.get_analyzed_dataframe',
                 return_value=(df, 1))
    ...
    uptradingbot.config['telegram']['notification_settings'] = {'buy': 'off'}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    uptradingbot.config['telegram']['notification_settings'] = {'buy': 'silent'}
    caplog.clear()
    msg_mock.reset_mock()
...
async def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    bot = MagicMock()
    bot.send_message = AsyncMock()
    uptradingbot = get_patched_uptradingbot(mocker, default_conf)
    rpc = RPC(uptradingbot)
    ...
    def init_telegram(uptradingbot):
        telegram = Telegram(rpc, default_conf)
        telegram._app = MagicMock()
        telegram._app.bot = bot
        return telegram
    ...
    uptradingbot.config['telegram']['enabled'] = True
    telegram = init_telegram(uptradingbot)
    await telegram._send_msg('test')
    ...
    uptradingbot.config['telegram']['enabled'] = True
    uptradingbot.config['telegram']['keyboard'] = invalid_keys_list
    ...
    uptradingbot.config['telegram']['enabled'] = True
    uptradingbot.config['telegram']['keyboard'] = custom_keys_list
    telegram = init_telegram(uptradingbot)
    await telegram._send_msg('test')
    used_keyboard = bot.send_message.call_args[1]['reply_markup']
...
async def test_change_market_direction(default_conf, mocker, update) -> None:
    telegram, _, msg_mock = get_telegram_testobject(mocker, default_conf)
    assert telegram._rpc._uptrading.strategy.market_direction == MarketDirection.NONE
    context = MagicMock()
    context.args = ["long"]
    await telegram._changemarketdir(update, context)
    assert telegram._rpc._uptrading.strategy.market_direction == MarketDirection.LONG
    context = MagicMock()
