...
from uptrading.loggers import setup_logging, setup_logging_pre
...
def botclient(default_conf, mocker):
    ...
    rpc = RPC(ftbot)
    mocker.patch('uptrading.rpc.api_server.ApiServer.start_api', MagicMock())
    apiserver = None
    try:
...
def test_api_ui_fallback(botclient, mocker):
    ...
    assert rc.status_code == 200
    assert '`uptrading install-ui`' in rc.text
    ...
    assert '`uptrading install-ui`' in rc.text or '<!DOCTYPE html>' in rc.text
...
def test_api_ui_version(botclient, mocker):
    ...
    mocker.patch('uptrading.commands.deploy_commands.read_ui_version', return_value='0.1.2')
...
def test_api_auth():
    with pytest.raises(ValueError):
        create_token({'identity': {'u': 'Uptrading'}}, 'secret1234', token_type="NotATokenType")

    token = create_token({'identity': {'u': 'Uptrading'}}, 'secret1234')
...
def test_api_ws_auth(botclient):
    ...
    jwt_token = create_token({'identity': {'u': 'Uptrading'}}, jwt_secret)
...
def test_api__init__(default_conf, mocker):
    ...
    mocker.patch('uptrading.rpc.telegram.Telegram._init')
    mocker.patch('uptrading.rpc.api_server.webserver.ApiServer.start_api', MagicMock())
    apiserver = ApiServer(default_conf)
    apiserver.add_rpc_handler(RPC(get_patched_uptradingbot(mocker, default_conf)))
...
def test_api_UvicornServer(mocker):
    thread_mock = mocker.patch('uptrading.rpc.api_server.uvicorn_threaded.threading.Thread')
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch('uptrading.rpc.api_server.uvicorn_threaded.UvicornServer.serve',
                              get_mock_coro(None))
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host='127.0.0.1'))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    mocker.patch('uptrading.rpc.api_server.webserver.UvicornServer', server_mock)
    ...
    caplog.clear()
    mocker.patch('uptrading.rpc.api_server.webserver.UvicornServer',
...
def test_api_show_config(botclient):
    ...
    assert response['bot_name'] == 'uptrading'
...
def test_api_pair_history(botclient, mocker):
    ftbot, client = botclient
    timeframe = '5m'
    lfm = mocker.patch('uptrading.strategy.interface.IStrategy.load_upAi_model')
    # No pair
    rc = client_get(client,
...
def test_api_plot_config(botclient, mocker):
    ...
    rc = client_get(client, f"{BASE_URI}/plot_config?strategy=upai_test_classifier")
    assert_response(rc)
    res = rc.json()
    ...
    mocker.patch('uptrading.rpc.api_server.api_v1.get_rpc_optional', return_value=None)
...
def test_api_strategies(botclient, tmpdir):
    ...
    assert rc.json() == {'strategies': [
        ...
        'upai_rl_test_strat',
        'upai_test_classifier',
        'upai_test_multimodel_classifier_strat',
        'upai_test_multimodel_strat',
        'upai_test_strat',
...
def test_api_upaimodels(botclient, tmpdir, mocker):
    ftbot, client = botclient
    ftbot.config['user_data_dir'] = Path(tmpdir)
    mocker.patch(
        "uptrading.resolvers.upaimodel_resolver.UpaiModelResolver.search_all_objects",
        return_value=[
    ...
    rc = client_get(client, f"{BASE_URI}/upaimodels")

    assert_response(rc)

    assert rc.json() == {'upaimodels': [
        'LightGBMClassifier',
...
def test_api_pairlists_evaluate(botclient, tmpdir, mocker):
    ...
    plm = mocker.patch('uptrading.rpc.api_server.api_background_tasks.__run_pairlist',
                       return_value=None)
    body = {
        "pairlists": [
            {"method": "StaticPairList", },
...
def test_api_backtesting(botclient, mocker, fee, caplog, tmpdir):
    try:
        ...
        mocker.patch('uptrading.optimize.backtesting.Backtesting.backtest_one_strategy',
...
def test_api_backtest_history(botclient, mocker, testdatadir):
    ftbot, client = botclient
    mocker.patch('uptrading.data.btanalysis._get_backtest_files',
                 return_value=[
...
def test_api_ws_subscribe(botclient, mocker):
    ...
    sub_mock = mocker.patch('uptrading.rpc.api_server.ws.WebSocketChannel.set_subscriptions')
...
def test_api_ws_send_msg(default_conf, mocker, caplog):
    try:
        ...
        mocker.patch('uptrading.rpc.telegram.Telegram._init')
        mocker.patch('uptrading.rpc.api_server.ApiServer.start_api')
        apiserver = ApiServer(default_conf)
        apiserver.add_rpc_handler(RPC(get_patched_uptradingbot(mocker, default_conf)))
