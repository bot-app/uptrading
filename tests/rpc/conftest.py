...
from uptrading.data.converter import ohlcv_to_dataframe, trades_list_to_df
from uptrading.edge import PairInfo
from uptrading.enums import CandleType, MarginMode, RunMode, SignalDirection, TradingMode
from uptrading.exchange import Exchange
from uptrading.exchange.exchange import timeframe_to_minutes
from uptrading.persistence import LocalTrade, Order, Trade, init_db
from uptrading.resolvers import ExchangeResolver
from uptrading.uptradingbot import UptradingBot
from uptrading.util import dt_ts
...
EXMS = 'uptrading.exchange.exchange.Exchange'
...
def generate_test_data(timeframe: str, size: int, start: str = '2020-07-05'):
    ...
    date = pd.date_range(start, periods=size, up=f'{tf_mins}min', tz='UTC')
    df = pd.DataFrame({
        'date': date,
        'open': base,
...
def patch_exchange(
    ...
    if mock_supported_modes:
        mocker.patch(
            f'uptrading.exchange.{id}.{id.capitalize()}._supported_trading_mode_margin_pairs',
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch('uptrading.wallets.Wallets.get_free', MagicMock(
        return_value=free
    ))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch('uptrading.uptradingbot.UptradingBot._refresh_active_whitelist',
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch('uptrading.edge.Edge.calculate', MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_uptradingbot(mocker, config) -> None:
    """
    ...
    mocker.patch('uptrading.uptradingbot.RPCManager._init', MagicMock())
    mocker.patch('uptrading.uptradingbot.RPCManager.send_msg', MagicMock())
    patch_whitelist(mocker, config)
    mocker.patch('uptrading.uptradingbot.ExternalMessageConsumer')
    mocker.patch('uptrading.configuration.config_validation._validate_consumers')


def get_patched_uptradingbot(mocker, config) -> UptradingBot:
    """
    ...
    :return: UptradingBot
    """
    patch_uptradingbot(mocker, config)
    return UptradingBot(config)
...
def patch_get_signal(
    ...
    uptrading.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    uptrading.strategy.get_exit_signal = patched_get_exit_signal

    uptrading.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_gc(mocker) -> None:
    mocker.patch("uptrading.main.gc_set_threshold")


@pytest.fixture(autouse=True)
def user_dir(mocker, tmpdir) -> Path:
    user_dir = Path(tmpdir) / "user_data"
    mocker.patch('uptrading.configuration.configuration.create_userdata_dir',
...
def patch_coingekko(mocker) -> None:
    ...
    mocker.patch.multiple(
        'uptrading.rpc.fiat_convert.CoinGeckoAPI',
