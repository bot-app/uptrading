...
from uptrading.enums import ExitType, RPCMessageType
from uptrading.rpc import RPC
from uptrading.rpc.discord import Discord
from uptrading.rpc.webhook import Webhook
...
def get_webhook_dict() -> dict:
    return {
        "enabled": True,
        "url": "https://maker.ifttt.com/trigger/uptrading_test/with/key/c764udvJ5jfSlswVRukZZ2/",
        "webhookentry": {
...
def test__init__(mocker, default_conf):
    ...
    webhook = Webhook(RPC(get_patched_uptradingbot(mocker, default_conf)), default_conf)
...
def test_send_msg_webhook(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("uptrading.rpc.webhook.Webhook._send_msg", msg_mock)
    webhook = Webhook(RPC(get_patched_uptradingbot(mocker, default_conf)), default_conf)
    # Test buy
    msg_mock = MagicMock()
    mocker.patch("uptrading.rpc.webhook.Webhook._send_msg", msg_mock)
...
def test__send_msg(default_conf, mocker, caplog):
    ...
    msg = {'value1': 'DEADBEEF',
           ...
           'value3': 'UPTRADING'}
    post = MagicMock()
    mocker.patch("uptrading.rpc.webhook.post", post)
...
def test_send_msg_discord(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("uptrading.rpc.webhook.Webhook._send_msg", msg_mock)
    discord = Discord(RPC(get_patched_uptradingbot(mocker, default_conf)), default_conf)
