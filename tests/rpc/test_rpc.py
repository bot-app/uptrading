...
from uptrading.enums import SignalDirection, State, TradingMode
from uptrading.exceptions import ExchangeError, InvalidOrderException, TemporaryError
from uptrading.persistence import Trade
from uptrading.persistence.pairlock_middleware import PairLocks
from uptrading.rpc import RPC, RPCException
...
from tests.conftest import (EXMS, create_mock_trades, create_mock_trades_usdt,
                            get_patched_uptradingbot, patch_get_signal)
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    # Fill open order ...
    uptradingbot.manage_open_orders()
    trades = Trade.get_open_trades()
    uptradingbot.exit_positions(trades)
...
def test__rpc_timeunit_profit(
    ...
    uptradingbot = get_patched_uptradingbot(mocker, default_conf_usdt)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    ...
    uptradingbot = get_patched_uptradingbot(mocker, default_conf)
    uptradingbot.strategy.order_types['stoploss_on_exchange'] = True
    create_mock_trades(fee, is_short)
    rpc = RPC(uptradingbot)
...
def test_rpc_force_exit(default_conf, ticker, fee, mocker) -> None:
    ...
    uptradingbot.config['max_open_trades'] = 3
    uptradingbot.enter_positions()
...
def test_rpc_force_entry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    uptradingbot.config['max_open_trades'] = 5
    ...
    uptradingbot.strategy.position_adjustment_enable = True
    ...
    pair = 'XRP/BTC'
    uptradingbot = get_patched_uptradingbot(mocker, default_conf)
    uptradingbot.config['stake_amount'] = 0
    patch_get_signal(uptradingbot)
    rpc = RPC(uptradingbot)
    pair = 'TKN/BTC'
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    uptradingbot = get_patched_uptradingbot(mocker, edge_conf)
