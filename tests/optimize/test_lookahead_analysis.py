...
from uptrading.commands.optimize_commands import start_lookahead_analysis
from uptrading.data.history import get_timerange
from uptrading.exceptions import OperationalException
from uptrading.optimize.analysis.lookahead import Analysis, LookaheadAnalysis
...
def test_start_lookahead_analysis(mocker):
    ...
    mocker.patch.multiple(
        'uptrading.optimize.analysis.lookahead_helpers.LookaheadAnalysisSubFunctions',
...
def test_initialize_single_lookahead_analysis(lookahead_conf, mocker, caplog):
    ...
    start_mock = mocker.patch('uptrading.optimize.analysis.lookahead.LookaheadAnalysis.start')
    strategy_obj = {
        'name': "strategy_test_v3_with_lookahead_bias",
        'location': Path(lookahead_conf['strategy_path'], f"{lookahead_conf['strategy']}.py")
    }
