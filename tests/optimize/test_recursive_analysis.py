...
from uptrading.commands.optimize_commands import start_recursive_analysis
from uptrading.data.history import get_timerange
from uptrading.exceptions import OperationalException
from uptrading.optimize.analysis.recursive import RecursiveAnalysis
...
def test_start_recursive_analysis(mocker):
    ...
    mocker.patch.multiple(
        'uptrading.optimize.analysis.recursive_helpers.RecursiveAnalysisSubFunctions',
...
def test_initialize_single_recursive_analysis(recursive_conf, mocker, caplog):
    ...
    start_mock = mocker.patch('uptrading.optimize.analysis.recursive.RecursiveAnalysis.start')
    strategy_obj = {
        'name': "strategy_test_v3_recursive_issue",
        'location': Path(recursive_conf['strategy_path'], f"{recursive_conf['strategy']}.py")
    }
