...
from uptrading.commands.optimize_commands import setup_optimize_configuration, start_hyperopt
from uptrading.data.history import load_data
from uptrading.enums import ExitType, RunMode
from uptrading.exceptions import OperationalException
from uptrading.optimize.hyperopt import Hyperopt
from uptrading.optimize.hyperopt_auto import HyperOptAuto
from uptrading.optimize.hyperopt_tools import HyperoptTools
from uptrading.optimize.optimize_reports import generate_strategy_stats
from uptrading.optimize.space import SKDecimal
from uptrading.strategy import IntParameter
from uptrading.util import dt_utc
...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch('uptrading.optimize.hyperopt.Hyperopt.start', start_mock)
...
def test_start_no_data(mocker, hyperopt_conf, tmpdir) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('uptrading.data.history.load_pair_history', MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        'uptrading.optimize.hyperopt.get_timerange',
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('uptrading.optimize.hyperopt.Hyperopt.__init__', hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of uptrading Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch('uptrading.optimize.hyperopt.dump')
    dumper2 = mocker.patch('uptrading.optimize.hyperopt.Hyperopt._save_result')
    mocker.patch('uptrading.optimize.hyperopt.calculate_market_change', return_value=1.5)
    mocker.patch('uptrading.optimize.hyperopt.file_dump_json')

    mocker.patch('uptrading.optimize.backtesting.Backtesting.load_bt_data',
    ...
    parallel = mocker.patch(
        'uptrading.optimize.hyperopt.Hyperopt.run_optimizer_parallel',
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch('uptrading.optimize.hyperopt.Backtesting.backtest', return_value=backtest_result)
    mocker.patch('uptrading.optimize.hyperopt.get_timerange',
                 return_value=(dt_utc(2017, 12, 10), dt_utc(2017, 12, 13)))
    patch_exchange(mocker)
    mocker.patch.object(Path, 'open')
    mocker.patch('uptrading.configuration.config_validation.validate_config_schema')
    mocker.patch('uptrading.optimize.hyperopt.load', return_value={'XRP/BTC': None})
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch("uptrading.strategy.hyper.HyperStrategyMixin.load_params_from_file",
                 MagicMock(return_value={}))
    mocker.patch("uptrading.optimize.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("uptrading.optimize.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf, caplog) -> None:
    mocker.patch('uptrading.optimize.hyperopt.dump', MagicMock())
    mocker.patch('uptrading.optimize.hyperopt.file_dump_json')
    mocker.patch('uptrading.optimize.backtesting.Backtesting.load_bt_data',
                 MagicMock(return_value=(MagicMock(), None)))
    mocker.patch(
    ...
    mocker.patch('uptrading.optimize.hyperopt_auto.HyperOptAuto._generate_indicator_space',
                 return_value=[])
...
def test_in_strategy_auto_hyperopt_per_epoch(mocker, hyperopt_conf, tmpdir, fee) -> None:
    ...
    go = mocker.patch('uptrading.optimize.hyperopt.Hyperopt.generate_optimizer',
                      return_value={
                            'loss': 0.05,
...
def test_stake_amount_unlimited_max_open_trades(mocker, hyperopt_conf, tmpdir, fee) -> None:
    ...
    hyperopt = Hyperopt(hyperopt_conf)
    mocker.patch('uptrading.optimize.hyperopt.Hyperopt._get_params_dict',
                 return_value={
                     'max_open_trades': -1
                     })
