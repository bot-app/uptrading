...
from uptrading import constants
from uptrading.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from uptrading.configuration import TimeRange
from uptrading.data import history
from uptrading.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from uptrading.data.converter import clean_ohlcv_dataframe
from uptrading.data.dataprovider import DataProvider
from uptrading.data.history import get_timerange
from uptrading.enums import CandleType, ExitType, RunMode
from uptrading.exceptions import DependencyException, OperationalException
from uptrading.exchange import timeframe_to_next_date, timeframe_to_prev_date
from uptrading.optimize.backtest_caching import get_backtest_metadata_filename, get_strategy_run_id
...
from uptrading.util.datetime_helpers import dt_utc
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch('uptrading.optimize.backtesting.Backtesting.start', start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has('Starting uptrading in Backtesting mode', caplog)
...
def test_backtesting_start(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('uptrading.data.history.get_timerange', get_timerange)
    patch_exchange(mocker)
    mocker.patch('uptrading.optimize.backtesting.Backtesting.backtest')
    mocker.patch('uptrading.optimize.backtesting.generate_backtest_stats')
    mocker.patch('uptrading.optimize.backtesting.show_backtest_results')
    sbs = mocker.patch('uptrading.optimize.backtesting.store_backtest_stats')
    sbc = mocker.patch('uptrading.optimize.backtesting.store_backtest_analysis_results')
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch('uptrading.data.history.history_utils.load_pair_history',
...
def test_backtesting_pairlist_list(default_conf, mocker, caplog, testdatadir, tickers) -> None:
    ...
    mocker.patch('uptrading.plugins.pairlistmanager.PairListManager.refresh_pairlist')
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch("uptrading.optimize.backtesting.price_to_precision", lambda p, *args: p)
    patch_exchange(mocker)
    ...
    # uptrading_liquidation_price = liq + (abs(open_rate - liq) * liq_buffer * side_1)
    ...
    mocker.patch("uptrading.wallets.Wallets.get_trade_stake_amount",
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch('uptrading.plugins.pairlistmanager.PairListManager.whitelist',
                 PropertyMock(return_value=['UNITTEST/BTC']))
    mocker.patch('uptrading.optimize.backtesting.Backtesting.backtest', backtestmock)
    text_table_mock = MagicMock()
    sell_reason_mock = MagicMock()
    strattable_mock = MagicMock()
    strat_summary = MagicMock()

    mocker.patch.multiple('uptrading.optimize.optimize_reports.bt_output',
                          text_table_bt_results=text_table_mock,
                          text_table_strategy=strattable_mock,
                          )
    mocker.patch.multiple('uptrading.optimize.optimize_reports.optimize_reports',
...
def test_backtest_start_multi_strat_caching(default_conf, mocker, caplog, testdatadir, run_id,
    ...
    mocker.patch('uptrading.plugins.pairlistmanager.PairListManager.whitelist',
                 PropertyMock(return_value=['UNITTEST/BTC']))
    mocker.patch('uptrading.optimize.backtesting.Backtesting.backtest', backtestmock)
    mocker.patch('uptrading.optimize.backtesting.show_backtest_results', MagicMock())
    ...
    mocker.patch.multiple('uptrading.data.btanalysis',
                          load_backtest_metadata=load_backtest_metadata,
                          load_backtest_stats=load_backtest_stats)
    mocker.patch('uptrading.optimize.backtesting.get_strategy_run_id', side_effect=['1', '2', '2'])
