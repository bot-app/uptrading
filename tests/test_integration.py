...
from uptrading.enums import ExitCheckTuple, ExitType, TradingMode
from uptrading.persistence import Trade
from uptrading.persistence.models import Order
from uptrading.rpc.rpc import RPC
from tests.conftest import EXMS, get_patched_uptradingbot, log_has_re, patch_get_signal
...
def test_may_execute_exit_stoploss_on_exchange_multi(default_conf, ticker, fee,
    ...
    * 3rd trade is sold via sell-signal
    ...
    mocker.patch("uptrading.strategy.interface.IStrategy.should_exit", should_sell_mock)
    wallets_mock = mocker.patch("uptrading.wallets.Wallets.update")
    mocker.patch("uptrading.wallets.Wallets.get_free", return_value=1000)
    mocker.patch("uptrading.wallets.Wallets.check_exit_amount", return_value=True)

    uptrading = get_patched_uptradingbot(mocker, default_conf)
    uptrading.strategy.order_types['stoploss_on_exchange'] = True
    # Switch ordertype to market to close trade immediately
    uptrading.strategy.order_types['exit'] = 'market'
    ...
    assert uptrading.strategy.confirm_trade_entry.call_count == 3
    uptrading.strategy.confirm_trade_entry.reset_mock()
    ...
    assert uptrading.strategy.confirm_trade_entry.call_count == 0
    assert uptrading.strategy.confirm_trade_exit.call_count == 1
    uptrading.strategy.confirm_trade_exit.reset_mock()
...
def test_forcebuy_last_unlimited(default_conf, ticker, fee, mocker, balance_ratio, result1) -> None:
    ...
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == 4
    assert uptrading.wallets.get_trade_stake_amount('XRP/BTC') == result1
    ...
    assert len(trades) == 5
    bals = uptrading.wallets.get_all_balances()
    ...
    bals2 = uptrading.wallets.get_all_balances()
    assert bals != bals2
    assert len(bals) == 6
    assert len(bals2) == 5
    assert 'LTC' in bals
...
def test_dca_short(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    ...
    patch_get_signal(uptrading, enter_long=False, enter_short=True)
    uptrading.enter_positions()
...
def test_dca_order_adjust(default_conf_usdt, ticker_usdt, leverage, fee, mocker) -> None:
    ...
    patch_get_signal(uptrading)
    uptrading.strategy.custom_entry_price = lambda **kwargs: ticker_usdt['ask'] * 0.96
    uptrading.strategy.leverage = MagicMock(return_value=leverage)
    uptrading.strategy.minimal_roi = {0: 0.2}
    ...
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=1.99)
    uptrading.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    assert trade.has_open_orders
    ...
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=120)
    mocker.patch(f'{EXMS}._dry_is_price_crossed', return_value=False)
    ...
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=1.95)
    ...
    # Fill DCA order
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=None)
    mocker.patch(f'{EXMS}._dry_is_price_crossed', return_value=True)
    uptrading.strategy.adjust_entry_price = MagicMock(side_effect=ValueError)
    ...
    # Full exit
    mocker.patch(f'{EXMS}._dry_is_price_crossed', return_value=False)
    uptrading.strategy.custom_exit = MagicMock(return_value='Exit now')
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=2.02)
    uptrading.process()
    trade = Trade.get_trades().first()
...
def test_dca_order_adjust_entry_replace_fails(
    ...
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=20)

    uptrading.process()

    assert uptrading.strategy.adjust_trade_position.call_count == 1
    trades = Trade.session.scalars(
        select(Trade)
        .where(Order.ft_is_open.is_(True))
        .where(Order.ft_order_side != "stoploss")
    ...
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=2.05)
    ...
    assert uptrading.strategy.adjust_entry_price.call_count == 2

    # Attempt order replacement - fails.
    uptrading.strategy.adjust_entry_price = MagicMock(return_value=1234)

    entry_mock = mocker.patch('uptrading.uptradingbot.UptradingBot.execute_entry',
                              return_value=False)
...
def test_dca_exiting(default_conf_usdt, ticker_usdt, fee, mocker, caplog, leverage) -> None:
    ...
    if not spot:
        ...
        default_conf_usdt['margin_mode'] = 'isolated'
    uptrading = get_patched_uptradingbot(mocker, default_conf_usdt)
    assert uptrading.trading_mode == TradingMode.FUTURES if not spot else TradingMode.SPOT
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    mocker.patch(f"{EXMS}.get_max_leverage", return_value=10)
    starting_amount = uptrading.wallets.get_total('USDT')
    ...
    assert trade.open_rate == 2.0
    assert pytest.approx(uptrading.wallets.get_free('USDT')) == starting_amount - 60
    if spot:
        assert pytest.approx(uptrading.wallets.get_total('USDT')) == starting_amount - 60
    else:
        assert uptrading.wallets.get_total('USDT') == starting_amount

    # Too small size
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=-59)
    uptrading.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 1
    assert pytest.approx(trade.stake_amount) == 60
    ...
    assert pytest.approx(uptrading.wallets.get_free('USDT')) == expected_profit

    if spot:
        assert pytest.approx(uptrading.wallets.get_total('USDT')) == expected_profit
    else:
        # total won't change in futures mode, only free / used will.
        assert uptrading.wallets.get_total('USDT') == starting_amount + trade.realized_profit
    caplog.clear()

    # Sell more than what we got (we got ~20 coins left)
    # First adjusts the amount to 20 - then rejects.
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=-50)
    uptrading.process()
    ...
    uptrading.strategy.adjust_trade_position = MagicMock(return_value=-0.01)
    uptrading.process()
    trade = Trade.get_trades().first()
    assert len(trade.orders) == 2
    assert trade.orders[-1].ft_order_side == 'sell'
