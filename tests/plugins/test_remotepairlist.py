...
from uptrading.plugins.pairlist.RemotePairList import RemotePairList
from uptrading.plugins.pairlistmanager import PairListManager
from tests.conftest import EXMS, get_patched_exchange, get_patched_uptradingbot, log_has
...
def test_gen_pairlist_with_local_file(mocker, rpl_config):
    ...
    mocker.patch('uptrading.plugins.pairlist.RemotePairList.open', return_value=mock_file)

    mock_file_path = mocker.patch('uptrading.plugins.pairlist.RemotePairList.Path')
    mock_file_path.exists.return_value = True

    jsonparse = json.loads(mock_file.read.return_value)
    mocker.patch('uptrading.plugins.pairlist.RemotePairList.rapidjson.load', return_value=jsonparse)
...
def test_fetch_pairlist_mock_response_html(mocker, rpl_config):
    ...
    mocker.patch("uptrading.plugins.pairlist.RemotePairList.requests.get",
...
def test_remote_pairlist_init_no_pairlist_url(mocker, rpl_config):
    ...
    with pytest.raises(OperationalException, match=r'`pairlist_url` not specified.'
        ...
        get_patched_uptradingbot(mocker, rpl_config)
