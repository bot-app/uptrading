...
from uptrading.constants import AVAILABLE_PAIRLISTS
from uptrading.data.dataprovider import DataProvider
from uptrading.enums import CandleType, RunMode
from uptrading.exceptions import OperationalException
from uptrading.persistence import LocalTrade, Trade
from uptrading.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist, expand_pairlist
from uptrading.plugins.pairlistmanager import PairListManager
from uptrading.resolvers import PairListResolver
from tests.conftest import (EXMS, create_mock_trades_usdt, get_patched_exchange,
                            get_patched_uptradingbot, log_has, log_has_re, num_log_has)
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    uptrading = get_patched_uptradingbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = uptrading.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(uptrading.exchange, default_conf, MagicMock())
    with pytest.raises(OperationalException,
                       match=r"Impossible to load Pairlist 'NonexistingPairList'. "
                             r"This class does not exist or contains Python code errors."):
        PairListResolver.load_pairlist('NonexistingPairList', uptrading.exchange, plm,
                                       default_conf, {}, 1)
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f'{EXMS}.markets', PropertyMock(return_value=markets))
    uptrading.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ['ETH/BTC', 'TKN/BTC']
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(uptrading.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (static_pl_conf['exchange']['pair_whitelist'] ==
            uptrading.config['exchange']['pair_whitelist'])
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(uptrading.pairlists.whitelist)
    assert static_pl_conf['exchange']['pair_blacklist'] == uptrading.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(uptrading.pairlists.whitelist)
    assert static_pl_conf['exchange']['pair_blacklist'] == uptrading.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = uptrading.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    uptrading = get_patched_uptradingbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_uptradingbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    uptrading.pairlists.refresh_pairlist()
    assert whitelist == uptrading.pairlists.whitelist

    whitelist_conf['pairlists'] = [{'method': 'VolumePairList'}]
    with pytest.raises(OperationalException,
                       match=r'`number_assets` not specified. Please check your configuration '
                             r'for "pairlist.config.number_assets"'):
        PairListManager(uptrading.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        'uptrading.plugins.pairlistmanager.PairListManager',
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    uptrading = get_patched_uptradingbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_uptradingbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(mocker, whitelist_conf, shitcoinmarkets, tickers,
    ...
    mocker.patch.multiple('uptrading.persistence.Trade',
    ...
    if whitelist_result == 'filter_at_the_beginning':
        ...
        uptrading.pairlists.refresh_pairlist()
        whitelist = uptrading.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(OperationalException,
        ...
        get_patched_uptradingbot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert uptrading.pairlists.name_list == [pairlist]
    assert pairlist in str(uptrading.pairlists.short_desc())
    assert isinstance(uptrading.pairlists.whitelist, list)
    assert isinstance(uptrading.pairlists.blacklist, list)
...
@pytest.mark.parametrize("whitelist,log_message", [
    ...
    (['ETH/BTC', 'TKN/BTC', 'XLTCUSDT'], "is not tradable with Uptrading"),
])
def test__whitelist_for_active_markets(mocker, whitelist_conf, markets, pairlist, whitelist, caplog,
    ...
    # Assign starting whitelist
    pairlist_handler = uptrading.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException,
        ...
        get_patched_uptradingbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    uptrading = get_patched_uptradingbot(mocker, whitelist_conf)
    assert len(uptrading.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    uptrading.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(uptrading.pairlists._pairlist_handlers[0]._pair_cache) == 1
    uptrading.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        uptrading = get_patched_uptradingbot(mocker, whitelist_conf_agefilter)
        assert uptrading.exchange.refresh_latest_ohlcv.call_count == 0
        uptrading.pairlists.refresh_pairlist()
        assert len(uptrading.pairlists.whitelist) == 3
        assert uptrading.exchange.refresh_latest_ohlcv.call_count > 0

        uptrading.pairlists.refresh_pairlist()
        assert len(uptrading.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert uptrading.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        uptrading.pairlists.refresh_pairlist()
        assert len(uptrading.pairlists.whitelist) == 3
        assert uptrading.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f'{EXMS}.refresh_latest_ohlcv', return_value=ohlcv_data)
        uptrading.pairlists.refresh_pairlist()
        assert len(uptrading.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(mocker, markets, default_conf, tickers, ohlcv_history,
    ...
    assert uptrading.exchange.refresh_latest_ohlcv.call_count == 0
    uptrading.pairlists.refresh_pairlist()
    assert len(uptrading.pairlists.whitelist) == expected_length
    assert uptrading.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = uptrading.exchange.refresh_latest_ohlcv.call_count
    uptrading.pairlists.refresh_pairlist()
    assert len(uptrading.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert uptrading.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_pricefilter_desc(mocker, whitelist_conf, markets, pairlistconfig,
    ...
    if desc_expected is not None:
        uptrading = get_patched_uptradingbot(mocker, whitelist_conf)
        short_desc = str(uptrading.pairlists.short_desc())
...
def test_performance_filter(mocker, whitelist_conf, pairlists, pair_allowlist, overall_performance,
    ...
    uptrading = get_patched_uptradingbot(mocker, allowlist_conf)
    mocker.patch.multiple(EXMS,
                          get_tickers=tickers,
                          markets=PropertyMock(return_value=markets)
                          )
    ...
    uptrading.pairlists.refresh_pairlist()
    allowlist = uptrading.pairlists.whitelist
