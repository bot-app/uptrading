...
from uptrading.enums import ExitType
from uptrading.persistence import PairLocks, Trade
from uptrading.persistence.trade_model import Order
from uptrading.plugins.protectionmanager import ProtectionManager
from tests.conftest import get_patched_uptradingbot, log_has_re
...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in uptrading.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog, is_short):
    ...
    assert not uptrading.protections.global_stop()
    ...
    assert uptrading.protections.global_stop()
    ...
    uptrading.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair, only_per_side):
    ...
    message = r"Trading stopped due to .*"
    pair = 'XRP/BTC'
    assert not uptrading.protections.stop_per_pair(pair)
    assert not uptrading.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not uptrading.protections.stop_per_pair(pair)
    assert uptrading.protections.global_stop() != only_per_pair
    ...
    uptrading.protections.stop_per_pair(pair)
    assert uptrading.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not uptrading.protections.global_stop()
    assert not uptrading.protections.stop_per_pair('XRP/BTC')
    ...
    assert not uptrading.protections.global_stop()
    assert uptrading.protections.stop_per_pair('XRP/BTC')
    ...
    assert uptrading.protections.stop_per_pair('ETH/BTC')
...
def test_LowProfitPairs(mocker, default_conf, fee, caplog, only_per_side):
    ...
    Trade.commit()
    assert uptrading.protections.stop_per_pair('XRP/BTC') != only_per_side
    ...
    assert uptrading.protections.global_stop() != only_per_side
    assert uptrading.protections.stop_per_pair('XRP/BTC') != only_per_side
...
def test_protection_manager_desc(mocker, default_conf, protectionconf,
    ...
    short_desc = str(uptrading.protections.short_desc())
