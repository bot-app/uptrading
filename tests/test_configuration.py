...
from uptrading.configuration import Configuration, validate_config_consistency
from uptrading.configuration.config_validation import validate_config_schema
from uptrading.configuration.deprecated_settings import (check_conflicting_settings,
                                                         process_deprecated_setting,
                                                         process_removed_setting,
                                                         process_temporary_deprecated_settings)
from uptrading.configuration.environment_vars import flat_vars_to_nested_dict
from uptrading.configuration.load_config import (load_config_file, load_file, load_from_files,
                                                 log_config_error_range)
from uptrading.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch('uptrading.configuration.load_config.Path.open', mocker.mock_open(
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('uptrading.configuration.load_config.Path.open',
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        'uptrading.configuration.load_config.load_config_file',
        configsmock
    )
...
def test_from_config(default_conf, mocker, caplog) -> None:
    ...
    config_files = [conf1, conf2]
    mocker.patch('uptrading.configuration.configuration.create_datadir', lambda c, x: x)

    configsmock = MagicMock(side_effect=config_files)
    mocker.patch('uptrading.configuration.load_config.load_config_file', configsmock)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch('uptrading.configuration.configuration.create_datadir', lambda c, x: x)
    mocker.patch('uptrading.configuration.configuration.load_from_files', configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        'uptrading.configuration.configuration.Path.open',
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch(
        'uptrading.configuration.configuration.create_datadir',
        lambda c, x: x
    )
    mocker.patch(
        'uptrading.configuration.configuration.create_userdata_dir',
    ...
    assert log_has('Using user-data directory: {} ...'.format(Path("/tmp/uptrading")), caplog)
    assert 'user_data_dir' in config
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('uptrading.loggers.set_loggers', MagicMock)
    arglist = ['trade', '-vvv']
    args = Arguments(arglist).get_parsed_arg()
...
def test__validate_upai_include_timeframes(default_conf, caplog) -> None:
    conf = deepcopy(default_conf)
    conf.update({
            "upai": {
                "enabled": True,
    ...
    conf['upai']['feature_parameters']['include_timeframes'] = ["5m", "15m"]
    validate_config_consistency(conf)
    assert conf['upai']['feature_parameters']['include_timeframes'] == ["1m", "5m", "15m"]

    conf.update({'analyze_per_epoch': True})
    with pytest.raises(OperationalException,
                       match=r"Using analyze-per-epoch .* not supported with a UpAi strategy."):
        validate_config_consistency(conf)
...
def test_pairlist_resolving_fallback(mocker, tmpdir):
    ...
    mocker.patch("uptrading.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    ...
    test_args = {
        'UPTRADING__EXCHANGE__SOME_SETTING': 'true',
        'UPTRADING__EXCHANGE__SOME_FALSE_SETTING': 'false',
        'UPTRADING__EXCHANGE__CONFIG__whatever': 'sometime',
        'UPTRADING__EXIT_PRICING__PRICE_SIDE': 'bid',
        'UPTRADING__EXIT_PRICING__cccc': '500',
        'UPTRADING__STAKE_AMOUNT': '200.05',
        'UPTRADING__TELEGRAM__CHAT_ID': '2151',
    ...
    assert log_has("Loading variable 'UPTRADING__EXCHANGE__SOME_SETTING'", caplog)
    assert not log_has("Loading variable 'NOT_RELEVANT'", caplog)


def test_setup_hyperopt_upai(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch(
        'uptrading.configuration.configuration.create_datadir',
        lambda c, x: x
...
def test_setup_upai_backtesting(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch(
        'uptrading.configuration.configuration.create_datadir',
        lambda c, x: x
    ...
    with pytest.raises(
        OperationalException, match=r".*--upai-backtest-live-models parameter is only.*"
    ):
        validate_config_consistency(config)
    ...
    conf['timerange'] = None
    conf['upai_backtest_live_models'] = False

    with pytest.raises(
        OperationalException, match=r".* pass --timerange if you intend to use UpAi .*"
    ):
        validate_config_consistency(conf)
