...
from uptrading.commands import start_plot_dataframe, start_plot_profit
from uptrading.configuration import TimeRange
from uptrading.data import history
from uptrading.data.btanalysis import load_backtest_data
from uptrading.data.metrics import create_cum_profit
from uptrading.exceptions import OperationalException
from uptrading.plot.plotting import (add_areas, add_indicators, add_profit, create_plotconfig,
...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch('uptrading.plot.plotting.add_indicators',
                            MagicMock(side_effect=fig_generating_mock))
    trades_mock = mocker.patch('uptrading.plot.plotting.plot_trades',
                               MagicMock(side_effect=fig_generating_mock))
...
def test_generate_Plot_filename():
    ...
    assert fn == "uptrading-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog, user_dir):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("uptrading.plot.plotting.plot", MagicMock())
    store_plot_file(fig, filename="uptrading-plot-UNITTEST_BTC-5m.html",
                    directory=user_dir / "plot")

    expected_fn = str(user_dir / "plot/uptrading-plot-UNITTEST_BTC-5m.html")
...
def test_generate_profit_graph(testdatadir):
    ...
    assert fig.layout.title.text == "Uptrading Profit plot"
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("uptrading.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("uptrading.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config", "config_examples/config_bittrex.example.json",
        "--pairs", "ETH/BTC"
