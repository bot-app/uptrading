...
from uptrading.constants import DEFAULT_DB_PROD_URL
from uptrading.enums import TradingMode
from uptrading.exceptions import OperationalException
from uptrading.persistence import Trade, init_db
from uptrading.persistence.base import ModelBase
from uptrading.persistence.migrations import get_last_sequence_ids, set_sequence_ids
...
def test_init_custom_db_url(default_conf, tmpdir):
    ...
    filename = f"{tmpdir}/uptrading2_test.sqlite"
...
def test_init_prod_db(default_conf, mocker):
    ...
    create_engine_mock = mocker.patch('uptrading.persistence.models.create_engine', MagicMock())
...
def test_init_dryrun_db(default_conf, tmpdir):
    filename = f"{tmpdir}/uptrading2_prod.sqlite"
    assert not Path(filename).is_file()
    default_conf.update({
        'dry_run': True,
...
def test_migrate_new(mocker, default_conf, fee, caplog):
    ...
    mocker.patch('uptrading.persistence.models.create_engine', lambda *args, **kwargs: engine)
