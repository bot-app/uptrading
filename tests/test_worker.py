...
def test_worker_state(mocker, default_conf, markets) -> None:
    ...
    assert worker.uptrading.state is State.STOPPED


def test_worker_running(mocker, default_conf, caplog) -> None:
    mock_throttle = MagicMock()
    mocker.patch('uptrading.worker.Worker._throttle', mock_throttle)
    mocker.patch('uptrading.persistence.Trade.stoploss_reinitialization', MagicMock())
    ...
    assert worker.uptrading.strategy
    assert worker.uptrading.strategy.dp
    assert isinstance(worker.uptrading.strategy.dp, DataProvider)
...
def test_worker_stopped(mocker, default_conf, caplog) -> None:
    ...
    worker = get_patched_worker(mocker, default_conf)
    worker.uptrading.state = State.STOPPED
...
def test_throttle_sleep_time(mocker, default_conf, caplog) -> None:
    ...
    caplog.set_level(logging.DEBUG)
    worker = get_patched_worker(mocker, default_conf)
    sleep_mock = mocker.patch("uptrading.worker.Worker._sleep")
...
def test_worker_heartbeat_running(default_conf, mocker, caplog):
    ...
    worker.uptrading.state = State.RUNNING
