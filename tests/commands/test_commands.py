...
from uptrading.commands import (start_backtesting_show, start_convert_data, start_convert_trades,
...
from uptrading.commands.deploy_commands import (clean_ui_subdir, download_and_install_ui,
                                                get_ui_download_url, read_ui_version)
from uptrading.commands.list_commands import start_list_upAi_models
...
from uptrading.util import dt_floor_day, dt_now, dt_utc
...
def test_start_trading_fail(mocker, caplog):
    ...
    mocker.patch("uptrading.worker.Worker.run", MagicMock(side_effect=OperationalException))

    mocker.patch("uptrading.worker.Worker.__init__", MagicMock(return_value=None))

    exitmock = mocker.patch("uptrading.worker.Worker.exit", MagicMock())
    args = [
        'trade',
    ...
    exitmock.reset_mock()
    caplog.clear()
    mocker.patch("uptrading.worker.Worker.__init__", MagicMock(side_effect=OperationalException))
    start_trading(get_args(args))
...
def test_start_webserver(mocker, caplog):
    ...
    api_server_mock = mocker.patch("uptrading.rpc.api_server.ApiServer", )
...
def test_list_exchanges(capsys):
    ...
    assert re.match(r"Exchanges available for Uptrading.*", captured.out)
...
def test_create_datadir(caplog, mocker):
    ...
    cud = mocker.patch("uptrading.commands.deploy_commands.create_userdata_dir", MagicMock())
    csf = mocker.patch("uptrading.commands.deploy_commands.copy_sample_files", MagicMock())
    args = [
        "create-userdir",
        "--userdir",
        "/temp/uptrading/test"
...
def test_start_new_strategy(mocker, caplog):
    ...
    mocker.patch('uptrading.commands.deploy_commands.setup_utils_configuration')
...
def test_start_install_ui(mocker):
    clean_mock = mocker.patch('uptrading.commands.deploy_commands.clean_ui_subdir')
    get_url_mock = mocker.patch('uptrading.commands.deploy_commands.get_ui_download_url',
                                return_value=('https://example.com/whatever', '0.0.1'))
    download_mock = mocker.patch('uptrading.commands.deploy_commands.download_and_install_ui')
    mocker.patch('uptrading.commands.deploy_commands.read_ui_version', return_value=None)
    args = [
        "install-ui",
    ]
...
def test_clean_ui_subdir(mocker, tmpdir, caplog):
    mocker.patch("uptrading.commands.deploy_commands.Path.is_dir",
                 side_effect=[True, True])
    mocker.patch("uptrading.commands.deploy_commands.Path.is_file",
                 side_effect=[False, True])
    rd_mock = mocker.patch("uptrading.commands.deploy_commands.Path.rmdir")
    ul_mock = mocker.patch("uptrading.commands.deploy_commands.Path.unlink")

    mocker.patch("uptrading.commands.deploy_commands.Path.glob",
...
def test_download_and_install_ui(mocker, tmpdir):
    ...
    mocker.patch("uptrading.commands.deploy_commands.requests.get", return_value=requests_mock)

    mocker.patch("uptrading.commands.deploy_commands.Path.is_dir",
                 side_effect=[True, False])
    wb_mock = mocker.patch("uptrading.commands.deploy_commands.Path.write_bytes")
...
def test_get_ui_download_url(mocker):
    ...
    get_mock = mocker.patch("uptrading.commands.deploy_commands.requests.get",
...
def test_download_data_keyboardInterrupt(mocker, markets):
    dl_mock = mocker.patch('uptrading.commands.data_commands.download_data_main',
...
def test_download_data_timerange(mocker, markets):
    dl_mock = mocker.patch('uptrading.data.history.history_utils.refresh_backtest_ohlcv_data',
...
def test_download_data_no_exchange(mocker):
    mocker.patch('uptrading.data.history.history_utils.refresh_backtest_ohlcv_data',
                 MagicMock(return_value=["ETH/BTC", "XRP/BTC"]))
    patch_exchange(mocker)
    mocker.patch(f'{EXMS}.get_markets', return_value={})
    args = [
...
def test_download_data_trades(mocker):
    dl_mock = mocker.patch('uptrading.data.history.history_utils.refresh_backtest_trades_data',
                           MagicMock(return_value=[]))
    convert_mock = mocker.patch('uptrading.data.history.history_utils.convert_trades_to_ohlcv',
                                MagicMock(return_value=[]))
    patch_exchange(mocker)
    mocker.patch(f'{EXMS}.get_markets', return_value={})
    args = [
...
def test_start_convert_trades(mocker, caplog):
    convert_mock = mocker.patch('uptrading.commands.data_commands.convert_trades_to_ohlcv',
                                MagicMock(return_value=[]))
    patch_exchange(mocker)
    mocker.patch(f'{EXMS}.markets', PropertyMock(return_value={}))
    args = [
...
def test_start_list_upAi_models(capsys):
...
def test_hyperopt_list(mocker, capsys, caplog, saved_hyperopt_results, tmpdir):
    ...
    mocker.patch(
        'uptrading.optimize.hyperopt_tools.HyperoptTools._test_hyperopt_results_exist',
        return_value=True
    )
    ...
    mocker.patch(
        'uptrading.optimize.hyperopt_tools.HyperoptTools._read_results',
...
def test_hyperopt_show(mocker, capsys, saved_hyperopt_results):
    ...
    mocker.patch('uptrading.commands.hyperopt_commands.show_backtest_result')
...
def test_convert_data(mocker, testdatadir):
    ohlcv_mock = mocker.patch("uptrading.commands.data_commands.convert_ohlcv_format")
    trades_mock = mocker.patch("uptrading.commands.data_commands.convert_trades_format")
    args = [
        "convert-data",
        "--format-from",
        "json",
...
def test_show_trades(mocker, fee, capsys, caplog):
    mocker.patch("uptrading.persistence.init_db")
...
def test_backtesting_show(mocker, testdatadir, capsys):
    sbr = mocker.patch('uptrading.optimize.optimize_reports.show_backtest_results')
    args = [
        "backtesting-show",
        "--export-filename",
...
def test_start_strategy_updater(mocker, tmpdir):
    sc_mock = mocker.patch('uptrading.commands.strategy_utils_commands.start_conversion')
