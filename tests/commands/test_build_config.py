...
from uptrading.commands.build_config_commands import (ask_user_config, ask_user_overwrite,
...
def test_start_new_config(mocker, caplog, exchange):
    ...
    mocker.patch('uptrading.commands.build_config_commands.ask_user_overwrite', return_value=True)
    ...
    sample_selections = {
        ...
        'api_server_username': 'uptradingr',
        'api_server_password': 'MoneyMachine',
    }
    mocker.patch('uptrading.commands.build_config_commands.ask_user_config',
...
def test_start_new_config_exists(mocker, caplog):
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch('uptrading.commands.build_config_commands.ask_user_overwrite', return_value=False)
    args = [
        "new-config",
        "--config",
        "coolconfig.json"
...
def test_ask_user_overwrite(mocker):
    ...
    """
    prompt_mock = mocker.patch('uptrading.commands.build_config_commands.prompt',
