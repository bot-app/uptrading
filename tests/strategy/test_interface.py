...
from uptrading.constants import CUSTOM_TAG_MAX_LENGTH
from uptrading.data.dataprovider import DataProvider
from uptrading.data.history import load_data
from uptrading.enums import ExitCheckTuple, ExitType, HyperoptState, SignalDirection
from uptrading.exceptions import OperationalException, StrategyError
from uptrading.optimize.hyperopt_tools import HyperoptStateContainer
...
from uptrading.strategy.hyper import detect_parameters
from uptrading.strategy.parameters import (BaseParameter, BooleanParameter, CategoricalParameter,
...
def test_upai_not_initialized(default_conf) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    strategy.ft_bot_start()
    with pytest.raises(OperationalException, match=r'upAi is not enabled\.'):
        strategy.upai.start()


def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch('uptrading.strategy.interface.IStrategy.advise_indicators')
    timerange = TimeRange.parse_timerange('1510694220-1510700340')
    data = load_data(testdatadir, '1m', ['UNITTEST/BTC'], timerange=timerange,
                     fill_up_missing=True)
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch('uptrading.strategy.hyper.HyperoptTools.load_params',
