...
from uptrading.edge import Edge, PairInfo
from uptrading.enums import ExitType
from uptrading.exceptions import OperationalException
from uptrading.util.datetime_helpers import dt_ts, dt_utc
from tests.conftest import EXMS, get_patched_uptradingbot, log_has
...
def test_edge_results(edge_conf, mocker, caplog, data) -> None:
    ...
    """
    uptrading = get_patched_uptradingbot(mocker, edge_conf)
    edge = Edge(edge_conf, uptrading.exchange, uptrading.strategy)
...
def test_edge_process_downloaded_data(mocker, edge_conf):
    ...
    mocker.patch('uptrading.edge.edge_positioning.refresh_data', MagicMock())
    mocker.patch('uptrading.edge.edge_positioning.load_data', mocked_load_data)
    edge = Edge(edge_conf, uptrading.exchange, uptrading.strategy)
...
def test_edge_process_no_data(mocker, edge_conf, caplog):
    ...
    mocker.patch(f'{EXMS}.get_fee', MagicMock(return_value=0.001))
    mocker.patch('uptrading.edge.edge_positioning.refresh_data', MagicMock())
    mocker.patch('uptrading.edge.edge_positioning.load_data', MagicMock(return_value={}))
    edge = Edge(edge_conf, uptrading.exchange, uptrading.strategy)
...
def test_edge_process_no_trades(mocker, edge_conf, caplog):
    ...
    mocker.patch('uptrading.edge.edge_positioning.refresh_data', )
    mocker.patch('uptrading.edge.edge_positioning.load_data', mocked_load_data)
    # Return empty
    mocker.patch('uptrading.edge.Edge._find_trades_for_stoploss_range', return_value=[])
    edge = Edge(edge_conf, uptrading.exchange, uptrading.strategy)
...
def test_edge_process_no_pairs(mocker, edge_conf, caplog):
    ...
    mocker.patch('uptrading.uptradingbot.validate_config_consistency')
...
def test_edge_init_error(mocker, edge_conf,):
    ...
    with pytest.raises(OperationalException,  match='Edge works only with unlimited stake amount'):
        get_patched_uptradingbot(mocker, edge_conf)
...
def test_process_expectancy(mocker, edge_conf, fee, risk_reward_ratio, expectancy):
    ...
    uptrading.exchange.get_fee = get_fee
    edge = Edge(edge_conf, uptrading.exchange, uptrading.strategy)
...
def test_process_expectancy_remove_pumps(mocker, edge_conf, fee,):
    ...
    uptrading.exchange.get_fee = fee
    edge = Edge(edge_conf, uptrading.exchange, uptrading.strategy)
