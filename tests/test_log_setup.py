...
from uptrading.loggers import (FTBufferingHandler, FTStdErrStreamHandler, set_loggers,
                               setup_logging, setup_logging_pre)
from uptrading.loggers.set_log_levels import (reduce_verbosity_for_bias_tester,
...
def test_reduce_verbosity():
    ...
    prior_level = logging.getLogger('uptrading').getEffectiveLevel()

    assert logging.getLogger('uptrading.resolvers').getEffectiveLevel() == logging.WARNING
    assert logging.getLogger('uptrading.strategy.hyper').getEffectiveLevel() == logging.WARNING
    # base level wasn't changed
    assert logging.getLogger('uptrading').getEffectiveLevel() == prior_level

    restore_verbosity_for_bias_tester()

    assert logging.getLogger('uptrading.resolvers').getEffectiveLevel() == prior_level
    assert logging.getLogger('uptrading.strategy.hyper').getEffectiveLevel() == prior_level
    assert logging.getLogger('uptrading').getEffectiveLevel() == prior_level
    # base level wasn't changed
