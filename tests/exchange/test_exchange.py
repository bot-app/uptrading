...
from uptrading.enums import CandleType, MarginMode, TradingMode
from uptrading.exceptions import (DDosProtection, DependencyException, ExchangeError,
                                  InsufficientFundsError, InvalidOrderException,
                                  OperationalException, PricingError, TemporaryError)
from uptrading.exchange import (Binance, Bittrex, Exchange, Kraken, market_is_active,
                                timeframe_to_prev_date)
from uptrading.exchange.common import (API_FETCH_ORDER_RETRY_COUNT, API_RETRY_COUNT,
                                       calculate_backoff, remove_exchange_credentials)
from uptrading.resolvers.exchange_resolver import ExchangeResolver
from uptrading.util import dt_now, dt_ts
...
def ccxt_exceptionhandlers(mocker, default_conf, api_mock, exchange_name,
    ...
    with patch('uptrading.exchange.common.time.sleep'):
...
async def async_ccxt_exception(mocker, default_conf, api_mock, fun, mock_ccxt_fun,
    ...
    with patch('uptrading.exchange.common.asyncio.sleep', get_mock_coro(None)):
        with pytest.raises(DDosProtection):
...
def test_validate_timeframes_failed(default_conf, mocker):
    ...
    with pytest.raises(OperationalException,
                       match=r"Timeframes < 1m are currently not supported by Uptrading."):
        Exchange(default_conf)
...
async def test__async_kucoin_get_candle_history(default_conf, mocker, caplog):
    from uptrading.exchange.common import _reset_logging_mixin
...
async def test___async_get_candle_history_sort(default_conf, mocker, exchange_name):
    ...
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    sort_mock = mocker.patch('uptrading.exchange.exchange.sorted', MagicMock(side_effect=sort_data))
    ...
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    # Reset sort mock
    sort_mock = mocker.patch('uptrading.exchange.sorted', MagicMock(side_effect=sort_data))
    # Test the OHLCV data sort
    res = await exchange._async_get_candle_history(
        'ETH/BTC', default_conf['timeframe'], CandleType.SPOT)
    assert res[0] == 'ETH/BTC'
...
def test_cancel_stoploss_order_with_result(default_conf, mocker, exchange_name):
    default_conf['dry_run'] = False
    mock_prefix = 'uptrading.exchange.gate.Gate'
    if exchange_name == 'okx':
        mock_prefix = 'uptrading.exchange.okx.Okx'
...
def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
    with patch('uptrading.exchange.common.time.sleep') as tm:
        with pytest.raises(InvalidOrderException):
            exchange.fetch_order(order_id='_', pair='TKN/BTC')
