...
from uptrading.exchange import (amount_to_contract_precision, amount_to_precision,
...
def test_check_exchange(default_conf, caplog) -> None:
    # Test an officially supported by Uptrading team exchange
    default_conf['runmode'] = RunMode.DRY_RUN
    default_conf.get('exchange').update({'name': 'BITTREX'})
    assert check_exchange(default_conf)
    assert log_has_re(r"Exchange .* is officially supported by the Uptrading development team\.",
                      caplog)
    caplog.clear()
    ...
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange \"binance\" is officially supported by the Uptrading development team\.",
        caplog)
    caplog.clear()
    ...
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange \"binanceus\" is officially supported by the Uptrading development team\.",
        caplog)
    caplog.clear()

    # Test an officially supported by Uptrading team exchange - with remapping
    default_conf.get('exchange').update({'name': 'okex'})
    assert check_exchange(default_conf)
    assert log_has_re(
        r"Exchange \"okex\" is officially supported by the Uptrading development team\.",
        caplog)
    caplog.clear()
    ...
    assert log_has_re(r"Exchange .* is known to the the ccxt library, available for the bot, "
                      ...
                      r"by the Uptrading development team\. .*", caplog)
    caplog.clear()

    # Test a 'bad' exchange, which known to have serious problems
    default_conf.get('exchange').update({'name': 'bitmex'})
    with pytest.raises(OperationalException,
                       match=r"Exchange .* will not work with Uptrading\..*"):
