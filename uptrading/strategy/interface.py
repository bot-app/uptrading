...
from uptrading.constants import CUSTOM_TAG_MAX_LENGTH, Config, IntOrInf, ListPairsWithTimeframes
from uptrading.data.dataprovider import DataProvider
from uptrading.enums import (CandleType, ExitCheckTuple, ExitType, MarketDirection, RunMode,
                             SignalDirection, SignalTagType, SignalType, TradingMode)
from uptrading.exceptions import OperationalException, StrategyError
from uptrading.exchange import timeframe_to_minutes, timeframe_to_next_date, timeframe_to_seconds
from uptrading.misc import remove_entry_exit_signals
from uptrading.persistence import Order, PairLocks, Trade
from uptrading.strategy.hyper import HyperStrategyMixin
from uptrading.strategy.informative_decorator import (InformativeData, PopulateIndicators,
                                                      _create_and_merge_informative_pair,
                                                      _format_pair_name)
from uptrading.strategy.strategy_wrapper import strategy_safe_wrapper
from uptrading.util import dt_now
from uptrading.wallets import Wallets
...
class IStrategy(ABC, HyperStrategyMixin):
    """
    Interface for uptrading strategies
    ...
    def load_upAi_model(self) -> None:
        if self.config.get('upai', {}).get('enabled', False):
            # Import here to avoid importing this if upAi is disabled
            from uptrading.resolvers.upaimodel_resolver import UpaiModelResolver
            from uptrading.upai.utils import download_all_data_for_training
            self.upai = UpaiModelResolver.load_upaimodel(self.config)
            self.upai_info = self.config["upai"]
            ...
            # Gracious failures if upAi is disabled but "start" is called.
            class DummyClass:
            ...
            self.upai = DummyClass()  # type: ignore
    ...
    def ft_bot_cleanup(self) -> None:
        """
        Clean up UpAi and child threads
        """
        self.upai.shutdown()
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
        ...
        For full documentation please go to https://www.egiftcard.cc/en/latest/strategy-advanced/
    ...
    def adjust_entry_price(self, trade: Trade, order: Optional[Order], pair: str,
        ...
        For full documentation please go to https://www.egiftcard.cc/en/latest/strategy-callbacks/
    ...
    def populate_any_indicators(self, pair: str, df: DataFrame, tf: str,
        ...
        This method is *only* used in UpaiDataKitchen class and therefore
        it is only called if UpAi is active.
    ...
    def feature_engineering_expand_all(self, dataframe: DataFrame, period: int,
        ...
        """
        *Only functional with UpAi enabled strategies*
        ...
        All features must be prepended with `%` to be recognized by UpAi internals.

        More details on how these config defined parameters accelerate feature engineering
        in the documentation at:

        https://www.egiftcard.cc/en/latest/upai-parameter-table/#feature-parameters

        https://www.egiftcard.cc/en/latest/upai-feature-engineering/#defining-the-features
    ...
    def feature_engineering_standard(
        ...
        *Only functional with UpAi enabled strategies*
        ...
        upai_feature_engineering_* functions.
        ...
        https://www.egiftcard.cc/en/latest/upai-feature-engineering
    ...
    def set_upai_targets(self, dataframe: DataFrame, metadata: Dict, **kwargs) -> DataFrame:
        """
        *Only functional with UpAi enabled strategies*
        Required function to set the targets for the model.
        All targets must be prepended with `&` to be recognized by the UpAi internals.
...
###
    ...
    def __informative_pairs_upai(self) -> ListPairsWithTimeframes:
        """
        Create informative-pairs needed for UpAi
        """
        if self.config.get('upai', {}).get('enabled', False):
            whitelist_pairs = self.dp.current_whitelist()
            candle_type = self.config.get('candle_type_def', CandleType.SPOT)
            corr_pairs = self.config["upai"]["feature_parameters"]["include_corr_pairlist"]
            informative_pairs = []
            for tf in self.config["upai"]["feature_parameters"]["include_timeframes"]:
    ...
    def gather_informative_pairs(self) -> ListPairsWithTimeframes:
        ...
        informative_pairs.extend(self.__informative_pairs_upai())
    ...
    def unlock_pair(self, pair: str) -> None:
        ...
        Not used by uptrading itself, but intended to be used if users lock pairs
