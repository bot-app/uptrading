...
from uptrading.constants import (DEFAULT_AMOUNT_RESERVE_PERCENT, NON_OPEN_EXCHANGE_STATES, BidAsk,
                                 BuySell, Config, EntryExit, ExchangeConfig,
                                 ListPairsWithTimeframes, MakerTaker, OBLiteral, PairWithTimeframe)
from uptrading.data.converter import clean_ohlcv_dataframe, ohlcv_to_dataframe, trades_dict_to_list
from uptrading.enums import OPTIMIZE_MODES, CandleType, MarginMode, PriceType, TradingMode
from uptrading.exceptions import (DDosProtection, ExchangeError, InsufficientFundsError,
                                  InvalidOrderException, OperationalException, PricingError,
                                  RetryableOrderError, TemporaryError)
from uptrading.exchange.common import (API_FETCH_ORDER_RETRY_COUNT, remove_exchange_credentials,
                                       retrier, retrier_async)
from uptrading.exchange.exchange_utils import (ROUND, ROUND_DOWN, ROUND_UP, CcxtModuleType,
...
from uptrading.exchange.types import OHLCVResponse, OrderBook, Ticker, Tickers
from uptrading.misc import (chunks, deep_merge_dicts, file_dump_json, file_load_json,
                            safe_value_fallback2)
from uptrading.plugins.pairlist.pairlist_helpers import expand_pairlist
from uptrading.util import dt_from_ts, dt_now
from uptrading.util.datetime_helpers import dt_humanize, dt_ts
...
class Exchange:
    ...
    def market_is_tradable(self, market: Dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by Uptrading.
    ...
    def validate_timeframes(self, timeframe: Optional[str]) -> None:
        ...
        if timeframe and timeframe_to_minutes(timeframe) < 1:
            raise OperationalException("Timeframes < 1m are currently not supported by Uptrading.")
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if uptrading can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by uptrading on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"Uptrading does not support {mm_value} {trading_mode.value} on {self.name}"
            )
    ...
    def fetch_dry_run_order(self, order_id) -> Dict[str, Any]:
        ...
        try:
            ...
            from uptrading.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            ...
            if market['inverse']:
                raise OperationalException(
                    "Uptrading does not yet support inverse contracts")
            ...
            raise OperationalException(
                "Uptrading only supports isolated futures for leverage trading")
