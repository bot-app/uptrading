...
def setup_analyze_configuration(args: Dict[str, Any], method: RunMode) -> Dict[str, Any]:
    ...
    if method in no_unlimited_runmodes.keys():
        from uptrading.data.btanalysis import get_latest_backtest_filename
...
def start_analysis_entries_exits(args: Dict[str, Any]) -> None:
    ...
    :return: None
    """
    from uptrading.data.entryexitanalysis import process_entry_exit_reasons

    # Initialize configuration
    config = setup_analyze_configuration(args, RunMode.BACKTEST)

    logger.info('Starting uptrading in analysis mode')
