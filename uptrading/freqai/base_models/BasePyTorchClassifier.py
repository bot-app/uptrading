...
from uptrading.upai.base_models.BasePyTorchModel import BasePyTorchModel
...
class BasePyTorchClassifier(BasePyTorchModel):
    ...
    under IStrategy.set_upai_targets method.

    for example, in your strategy:
    ```
        def set_upai_targets(self, dataframe: DataFrame, metadata: Dict, **kwargs):
    ...
    def get_class_names(self) -> List[str]:
        if not self.class_names:
            raise ValueError(
                ...
                "set self.upai.class_names = ['class a', 'class b', 'class c'] "
                "inside IStrategy.set_upai_targets method."
            )
    ...
    def train(
        ...
        if not self.upai_info.get("fit_live_predictions_candles", 0) or not self.live:
            dk.fit_labels()
