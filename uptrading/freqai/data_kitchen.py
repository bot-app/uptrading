...
from uptrading.data.converter import reduce_dataframe_footprint
from uptrading.exceptions import OperationalException
from uptrading.exchange import timeframe_to_seconds
from uptrading.strategy import merge_informative_pair
...
class UpaiDataKitchen:
    """
    Class designed to analyze data for a single pair. Employed by the IUpaiModel class.
    ...
    def __init__(
        ...
        self.config = config
        self.upai_config: Dict[str, Any] = config["upai"]
        ...
        self.live = live
        self.pair = pair
        self.keras: bool = self.upai_config.get("keras", False)
        self.set_all_pairs()
        self.backtest_live_models = config.get("upai_backtest_live_models", False)
        ...
        if not self.live:
            ...
            if not self.backtest_live_models:
                self.full_timerange = self.create_fulltimerange(
                    self.config["timerange"], self.upai_config.get("train_period_days", 0)
                )
                (self.training_timeranges, self.backtesting_timeranges) = self.split_timerange(
                    self.full_timerange,
                    config["upai"]["train_period_days"],
                    config["upai"]["backtest_period_days"],
                )

        self.data['extra_returns_per_train'] = self.upai_config.get('extra_returns_per_train', {})
        if not self.upai_config.get("data_kitchen_thread_count", 0):
            self.thread_count = max(int(psutil.cpu_count() * 2 - 2), 1)
        else:
            self.thread_count = self.upai_config["data_kitchen_thread_count"]
    ...
    def make_train_test_datasets(
        ...
        """
        feat_dict = self.upai_config["feature_parameters"]

        if 'shuffle' not in self.upai_config['data_split_parameters']:
            self.upai_config["data_split_parameters"].update({'shuffle': False})
        ...
        if self.upai_config.get('data_split_parameters', {}).get('test_size', 0.1) != 0:
            (
                train_features,
                test_features,
                train_labels,
                ...
                labels,
                weights,
                **self.config["upai"]["data_split_parameters"],
            )
        else:
        ...
        if self.upai_config['feature_parameters'].get('reverse_train_test_order', False):
    ...
    def set_weights_higher_recent(self, num_weights: int) -> npt.ArrayLike:
        ...
        """
        wfactor = self.config["upai"]["feature_parameters"]["weight_factor"]
    ...
    def get_predictions_to_append(self, predictions: DataFrame,
        ...
        if self.upai_config["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def create_fulltimerange(self, backtest_tr: str, backtest_period_days: int) -> str:
        ...
        if backtest_timerange.stopts == 0:
            ...
            raise OperationalException("UpAi backtesting does not allow open ended timeranges. "
    ...
    def check_if_model_expired(self, trained_timestamp: int) -> bool:
        ...
        max_time = self.upai_config.get("expiration_hours", 0)
        if max_time > 0:
    ...
    def check_if_new_training_required(
        ...
        timeframes = self.upai_config["feature_parameters"].get("include_timeframes")
        ...
        if trained_timestamp != 0:
            ...
            retrain = elapsed_time > self.upai_config.get("live_retrain_hours", 0)
            if retrain:
                trained_timerange.startts = int(
                    time - self.upai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                )
                ...
                data_load_timerange.startts = int(
                    time
                    - self.upai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                    - additional_seconds
                )
    ...
    def set_all_pairs(self) -> None:
        ...
        self.all_pairs = copy.deepcopy(
            self.upai_config["feature_parameters"].get("include_corr_pairlist", [])
        )
    ...
    def extract_corr_pair_columns_from_populated_indicators(
        ...
        pairs = self.upai_config["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def populate_features(self, dataframe: DataFrame, pair: str, strategy: IStrategy,
        ...
        """
        tfs: List[str] = self.upai_config["feature_parameters"].get("include_timeframes")
        ...
        for tf in tfs:
            ...
            for t in self.upai_config["feature_parameters"]["indicator_periods_candles"]:
            ...
            for n in range(self.upai_config["feature_parameters"]["include_shifted_candles"] + 1):
                if n == 0:
                    continue
    ...
    def use_strategy_to_populate_indicators(  # noqa: C901
        ...
        if not new_version:
            raise OperationalException(
                ...
                f"{DOCS_LINK}/strategy_migration/#upai-strategy \n"
                "And the feature_engineering_* documentation: \n"
                f"{DOCS_LINK}/upai-feature-engineering/"
                )

        tfs: List[str] = self.upai_config["feature_parameters"].get("include_timeframes")
        pairs: List[str] = self.upai_config["feature_parameters"].get(
        ...
        corr_pairs: List[str] = self.upai_config["feature_parameters"].get(
            "include_corr_pairlist", [])
        ...
        if self.live:
            dataframe = strategy.set_upai_targets(dataframe.copy(), metadata=metadata)
    ...
    def get_full_models_path(self, config: Config) -> Path:
        """
        Returns default UpAi model path
        :param config: Configuration dictionary
        """
        upai_config: Dict[str, Any] = config["upai"]
        return Path(
            config["user_data_dir"] / "models" / str(upai_config.get("identifier"))
        )
    ...
    def buffer_timerange(self, timerange: TimeRange):
        ...
        will be NaN and UpAi will automatically cut those off of the training
        dataset.
        """
        buffer = self.upai_config["feature_parameters"]["buffer_train_data_candles"]
        if buffer:
    ...
    def normalize_data(self, data_dictionary: Dict) -> Dict[Any, Any]:
        ...
        """
        logger.warning(f"Your custom IUpaiModel relies on the deprecated"
