...
from uptrading import misc
from uptrading.configuration import TimeRange
from uptrading.constants import DEFAULT_TRADES_COLUMNS, ListPairsWithTimeframes
from uptrading.data.converter import (clean_ohlcv_dataframe, trades_convert_types,
        ...
        if pairdf.empty:
            if warn_no_data:
                logger.warning(
                    ...
                    "Use `uptrading download-data` to download the data"
                )
            return True
        elif warn_price:
            candle_price_gap = 0
