...
from uptrading.constants import (FULL_DATAFRAME_THRESHOLD, Config, ListPairsWithTimeframes,
                                 PairWithTimeframe)
from uptrading.data.history import load_pair_history
from uptrading.enums import CandleType, RPCMessageType, RunMode
from uptrading.exceptions import ExchangeError, OperationalException
from uptrading.exchange import Exchange, timeframe_to_prev_date, timeframe_to_seconds
from uptrading.exchange.types import OrderBook
from uptrading.misc import append_candles_to_dataframe
from uptrading.rpc import RPCManager
from uptrading.rpc.rpc_types import RPCAnalyzedDFMsg
from uptrading.util import PeriodicCache
...
class DataProvider:
    ...
    def get_required_startup(self, timeframe: str, add_train_candles: bool = True) -> int:
        upai_config = self._config.get('upai', {})
        if not upai_config.get('enabled', False):
            return self._config.get('startup_candle_count', 0)
        else:
            startup_candles = self._config.get('startup_candle_count', 0)
            indicator_periods = upai_config['feature_parameters']['indicator_periods_candles']
            # make sure the startupcandles is at least the set maximum indicator periods
            self._config['startup_candle_count'] = max(startup_candles, max(indicator_periods))
            tf_seconds = timeframe_to_seconds(timeframe)
            train_candles = 0
            if add_train_candles:
                train_candles = upai_config['train_period_days'] * 86400 / tf_seconds
            total_candles = int(self._config['startup_candle_count'] + train_candles)
            logger.info(f'Increasing startup_candle_count for upai to {total_candles}')
