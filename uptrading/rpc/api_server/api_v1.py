...
from uptrading.data.history import get_datahandler
from uptrading.enums import CandleType, TradingMode
from uptrading.exceptions import OperationalException
from uptrading.rpc import RPC
from uptrading.rpc.api_server.api_schemas import (AvailablePairs, Balances, BlacklistPayload,
                                                  BlacklistResponse, Count, DailyWeeklyMonthly,
                                                  DeleteLockRequest, DeleteTrade,
                                                  ExchangeListResponse, ForceEnterPayload,
                                                  ForceEnterResponse, ForceExitPayload,
                                                  UpAiModelListResponse, Health, Locks, Logs,
                                                  OpenTradeSchema, PairHistory, PerformanceEntry,
                                                  Ping, PlotConfig, Profit, ResultMsg, ShowConfig,
                                                  Stats, StatusMsg, StrategyListResponse,
                                                  StrategyResponse, SysInfo, Version,
                                                  WhitelistResponse)
from uptrading.rpc.api_server.deps import get_config, get_exchange, get_rpc, get_rpc_optional
...
def show_config(rpc: Optional[RPC] = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._uptrading.state
        strategy_version = rpc._uptrading.strategy.version()
...
def pair_history(pair: str, timeframe: str, timerange: str, strategy: str,
    ...
    config.update({
        'strategy': strategy,
        'timerange': timerange,
        'upaimodel': upaimodel if upaimodel else config.get('upaimodel'),
    })
    try:
...
def list_exchanges(config=Depends(get_config)):
    from uptrading.exchange import list_available_exchanges
...
@router.get('/upaimodels', response_model=UpAiModelListResponse, tags=['upai'])
def list_upaimodels(config=Depends(get_config)):
    from uptrading.resolvers.upaimodel_resolver import UpaiModelResolver
    models = UpaiModelResolver.search_all_objects(
        config, False)
    models = sorted(models, key=lambda x: x['name'])

    return {'upaimodels': [x['name'] for x in models]}
