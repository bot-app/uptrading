...
from uptrading.data.btanalysis import (delete_backtest_result, get_backtest_result,
                                       get_backtest_resultlist, load_and_merge_backtest_result,
                                       update_backtest_metadata)
from uptrading.enums import BacktestState
from uptrading.exceptions import DependencyException, OperationalException
from uptrading.exchange.common import remove_exchange_credentials
from uptrading.misc import deep_merge_dicts, is_file_in_dir
from uptrading.rpc.api_server.api_schemas import (BacktestHistoryEntry, BacktestMetadataUpdate,
                                                  BacktestRequest, BacktestResponse)
from uptrading.rpc.api_server.deps import get_config
from uptrading.rpc.api_server.webserver_bgwork import ApiBG
from uptrading.rpc.rpc import RPCException
from uptrading.types import get_BacktestResultType_default
...
def __run_backtest_bg(btconfig: Config):
    from uptrading.optimize.optimize_reports import generate_backtest_stats, store_backtest_stats
...
async def api_start_backtest(
    ...
    if settings.get('upai', None) is not None:
        settings['upai'] = dict(settings['upai'])
...
def api_get_backtest():
    ...
    """
    from uptrading.persistence import LocalTrade
    if ApiBG.bgtask_running:
        return {
            "status": "running",
            "running": True,
