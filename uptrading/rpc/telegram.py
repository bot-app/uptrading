...
from uptrading.constants import DUST_PER_COIN, Config
from uptrading.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from uptrading.exceptions import OperationalException
from uptrading.misc import chunks, plural, round_coin_value
from uptrading.persistence import Trade
from uptrading.rpc import RPC, RPCException, RPCHandler
from uptrading.rpc.rpc_types import RPCSendMsg
from uptrading.util import dt_humanize
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != 'off':
            if candle_val == 'ohlc':
                analyzed_df, _ = self._rpc._uptrading.dataprovider.get_analyzed_dataframe(
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._uptrading.trading_mode != TradingMode.SPOT:
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._uptrading.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._uptrading.state)
