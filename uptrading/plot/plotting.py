...
from uptrading.data.btanalysis import (analyze_trade_parallelism, extract_trades_of_period,
                                       load_trades)
from uptrading.data.converter import trim_dataframe
from uptrading.data.dataprovider import DataProvider
from uptrading.data.history import get_timerange, load_data
from uptrading.data.metrics import (calculate_max_drawdown, calculate_underwater,
                                    combine_dataframes_with_mean, create_cum_profit)
from uptrading.enums import CandleType
from uptrading.exceptions import OperationalException
from uptrading.exchange import timeframe_to_prev_date, timeframe_to_seconds
from uptrading.misc import pair_to_filename
...
from uptrading.strategy import IStrategy
...
def generate_profit_graph(pairs: str, data: Dict[str, pd.DataFrame],
    ...
    fig['layout'].update(title="Uptrading Profit plot")
    fig['layout']['yaxis1'].update(title='Price')
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = 'uptrading-plot-' + pair_s + '-' + timeframe + '.html'
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(fig, filename='uptrading-profit-plot.html',
                    directory=config['user_data_dir'] / 'plot',
