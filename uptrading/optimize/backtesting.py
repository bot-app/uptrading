...
from uptrading.configuration import TimeRange, validate_config_consistency
from uptrading.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from uptrading.data import history
from uptrading.data.btanalysis import find_existing_backtest_stats, trade_list_to_dataframe
from uptrading.data.converter import trim_dataframe, trim_dataframes
from uptrading.data.dataprovider import DataProvider
from uptrading.enums import (BacktestState, CandleType, ExitCheckTuple, ExitType, RunMode,
                             TradingMode)
from uptrading.exceptions import DependencyException, OperationalException
from uptrading.exchange import (amount_to_contract_precision, price_to_precision,
                                timeframe_to_minutes, timeframe_to_seconds)
from uptrading.exchange.exchange import Exchange
from uptrading.mixins import LoggingMixin
from uptrading.optimize.backtest_caching import get_strategy_run_id
from uptrading.optimize.bt_progress import BTProgress
from uptrading.optimize.optimize_reports import (generate_backtest_stats, generate_rejected_signals,
                                                 generate_trade_signal_candles,
                                                 show_backtest_results,
                                                 store_backtest_analysis_results,
                                                 store_backtest_stats)
from uptrading.persistence import LocalTrade, Order, PairLocks, Trade
from uptrading.plugins.pairlistmanager import PairListManager
from uptrading.plugins.protectionmanager import ProtectionManager
from uptrading.resolvers import ExchangeResolver, StrategyResolver
from uptrading.strategy.interface import IStrategy
from uptrading.strategy.strategy_wrapper import strategy_safe_wrapper
from uptrading.types import BacktestResultType, get_BacktestResultType_default
from uptrading.util.binance_mig import migrate_binance_futures_data
...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Optional[Exchange] = None) -> None:
        ...
        if self.config.get('strategy_list'):
            if self.config.get('upai', {}).get('enabled', False):
                logger.warning("Using --strategy-list with UpAi REQUIRES all strategies "
        ...
        if self.config.get('upai_backtest_live_models', False):
            from uptrading.upai.utils import get_timerange_backtest_live_models
        ...
        if self.config.get('upai', {}).get('enabled', False):
            # For UpAi, increase the required_startup to includes the training data
    ...
    def _get_close_rate_for_roi(self, row: Tuple, trade: LocalTrade, exit: ExitCheckTuple,
        ...
        if roi is not None and roi_entry is not None:
            ...
            if (trade_dur == 0 and (
                ...
                # details: https: // github.com/bot-app/uptrading/issues/6261
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (ExitType.EXIT_SIGNAL, ExitType.CUSTOM_EXIT,
                ...
                if order_type == 'limit':
                    ...
                    # uptrading does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_price_and_stake(
        ...
        if order_type == 'limit':
            ...
            # which uptrading does not support in live.
