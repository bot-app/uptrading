...
from uptrading.constants import DOCS_LINK, Config
...
from uptrading.exchange import timeframe_to_seconds
from uptrading.upai.data_drawer import UpaiDataDrawer
from uptrading.upai.data_kitchen import UpaiDataKitchen
from uptrading.upai.utils import get_tb_logger, plot_feature_importance, record_params
...
class IUpaiModel(ABC):
    """
    Class containing all tools for training and prediction in the strategy.
    Base*PredictionModels inherit from this class.

    Record of contribution:
    UpAi was developed by a group of individuals who all contributed specific skillsets to the
    project.
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.upai_info: Dict[str, Any] = config["upai"]
        self.data_split_parameters: Dict[str, Any] = config.get("upai", {}).get(
            "data_split_parameters", {})
        self.model_training_parameters: Dict[str, Any] = config.get("upai", {}).get(
            "model_training_parameters", {})
        self.identifier: str = self.upai_info.get("identifier", "no_id_provided")
        self.retrain = False
        self.first = True
        self.set_full_path()
        self.save_backtest_models: bool = self.upai_info.get("save_backtest_models", True)
        if self.save_backtest_models:
            logger.info('Backtesting module configured to save all models.')

        self.dd = UpaiDataDrawer(Path(self.full_path), self.config)
        # set current candle to arbitrary historical date
        self.current_candle: datetime = datetime.fromtimestamp(637887600, tz=timezone.utc)
        self.dd.current_candle = self.current_candle
        self.scanning = False
        self.ft_params = self.upai_info["feature_parameters"]
        self.corr_pairlist: List[str] = self.ft_params.get("include_corr_pairlist", [])
        self.keras: bool = self.upai_info.get("keras", False)
        if self.keras and self.ft_params.get("DI_threshold", 0):
            self.ft_params["DI_threshold"] = 0
            logger.warning("DI threshold is not configured for Keras models yet. Deactivating.")

        self.CONV_WIDTH = self.upai_info.get('conv_width', 1)
        ...
        self.continual_learning = self.upai_info.get('continual_learning', False)
        ...
        self.activate_tensorboard: bool = self.upai_info.get('activate_tensorboard', True)
    ...
    def assert_config(self, config: Config) -> None:
        ...
        if not config.get("upai", {}):
            raise OperationalException("No upai parameters found in configuration file.")

    def start(self, dataframe: DataFrame, metadata: dict, strategy: IStrategy) -> DataFrame:
        """
        Entry point to the UpaiModel from a specific pair, it will train a new model if
        ...
        if self.live:
            ...
            self.dk = UpaiDataKitchen(self.config, self.live, metadata["pair"])
        ...
        # UpAi slides the window and sequentially builds the backtesting results before returning
        # the concatenated results for the full backtesting period back to the strategy.
        else:
            self.dk = UpaiDataKitchen(self.config, self.live, metadata["pair"])
            if not self.config.get("upai_backtest_live_models", False):
    ...
    def _start_scanning(self, strategy: IStrategy) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            dk = UpaiDataKitchen(self.config, self.live, pair)
            (
                retrain,
            ...
            if retrain:
                ...
                if self.upai_info.get('write_metrics_to_disk', False):
                    self.dd.save_metric_tracker_to_disk()

    def start_backtesting(
        self, dataframe: DataFrame, metadata: dict, dk: UpaiDataKitchen, strategy: IStrategy
    ) -> UpaiDataKitchen:
        """
        ...
        following the training window). UpAi slides the window and sequentially builds
        ...
        :param dk: UpaiDataKitchen = Data management/analysis tool associated to present pair only
        :param strategy: Strategy to train on
        :return:
            UpaiDataKitchen = Data management/analysis tool associated to present pair only
        """
    ...
    def start_live(
        self, dataframe: DataFrame, metadata: dict, strategy: IStrategy, dk: UpaiDataKitchen
    ) -> UpaiDataKitchen:
        """
        ...
        dk: UpaiDataKitchen = Data management/analysis tool associated to present pair only
        :returns:
        dk: UpaiDataKitchen = Data management/analysis tool associated to present pair only
        """

        if not strategy.process_only_new_candles:
            raise OperationalException("You are trying to use a UpAi strategy with "
                                       "process_only_new_candles = False. This is not supported "
                                       "by UpAi, and it is therefore aborting.")
    ...
    def build_strategy_return_arrays(
        self, dataframe: DataFrame, dk: UpaiDataKitchen, pair: str, trained_timestamp: int
    ) -> None:
        ...
        if pair not in self.dd.model_return_values:
            ...
            # allows UpUI to show full return values.
        ...
        if self.upai_info.get('fit_live_predictions_candles', 0) and self.live:
    ...
    def check_if_feature_list_matches_strategy(
        self, dk: UpaiDataKitchen
    ) -> None:
        """
        Ensure user is passing the proper feature set if they are reusing an `identifier` pointing
        to a folder holding existing models.
        :param dataframe: DataFrame = strategy provided dataframe
        :param dk: UpaiDataKitchen = non-persistent data container/analyzer for
        ...
        if dk.training_features_list != feature_list:
            raise OperationalException(
                ...
                "model. In case of --strategy-list, please be aware that UpAi "
                "requires all strategies to maintain identical "
                "feature_engineering_* functions"
            )

    def define_data_pipeline(self, threads=-1) -> Pipeline:
        ft_params = self.upai_info["feature_parameters"]
        pipe_steps = [
        ...
        sigma = self.upai_info["feature_parameters"].get('noise_standard_deviation', 0)
        if sigma:
    ...
    def model_exists(self, dk: UpaiDataKitchen) -> bool:
        """
    ...
    def extract_data_and_train_model(
        ...
        :param dk: UpaiDataKitchen = non-persistent data container for current coin/loop
    ...
    def set_initial_historic_predictions(
        self, pred_df: DataFrame, dk: UpaiDataKitchen, pair: str, strat_df: DataFrame
    ) -> None:
        """
        ...
        data. After that, UpAi will append new real predictions to the
        ...
        :param pred_df: DataFrame = the dataframe containing the predictions coming
            out of a model
        :param dk: UpaiDataKitchen = object containing methods for data analysis
        ...
        if self.upai_info['feature_parameters'].get('DI_threshold', 0) > 0:
    ...
    def fit_live_predictions(self, dk: UpaiDataKitchen, pair: str) -> None:
        """
        ...
        num_candles = self.upai_info.get("fit_live_predictions_candles", 100)
    ...
    def inference_timer(self, do: Literal['start', 'stop'] = 'start', pair: str = ''):
        """
        Timer designed to track the cumulative time spent in UpAi for one pass through
    ...
    def cache_corr_pairlist_dfs(self, dataframe: DataFrame, dk: UpaiDataKitchen) -> DataFrame:
        """
    ...
    def track_current_candle(self):
        ...
        equivalent to the latest candle seen by UpAi. If not, it
    ...
    def ensure_data_exists(self, len_dataframe_backtest: int,
        ...
        """
        if self.config.get("upai_backtest_live_models", False) and len_dataframe_backtest == 0:
    ...
    def backtesting_fit_live_predictions(self, dk: UpaiDataKitchen):
        """
        ...
        """
        fit_live_predictions_candles = self.upai_info.get("fit_live_predictions_candles", 0)
    ...
    def start_backtesting_from_historic_predictions(
        self, dataframe: DataFrame, metadata: dict, dk: UpaiDataKitchen
    ) -> UpaiDataKitchen:
        """
        :param dataframe: DataFrame = strategy passed dataframe
        :param metadata: Dict = pair metadata
    ...
    # See upai/prediction_models/CatboostPredictionModel.py for an example.

    @abstractmethod
    def train(self, unfiltered_df: DataFrame, pair: str,
              dk: UpaiDataKitchen, **kwargs) -> Any:
        """
    ...
    @abstractmethod
    def fit(self, data_dictionary: Dict[str, Any], dk: UpaiDataKitchen, **kwargs) -> Any:
        """
        Most regressors use the same function names and arguments e.g. user
        can drop in LGBMRegressor in place of CatBoostRegressor and all data
        management will be properly handled by Upai.
    ...
    @abstractmethod
    def predict(
        self, unfiltered_df: DataFrame, dk: UpaiDataKitchen, **kwargs
        ...
        :do_predict: np.array of 1s and 0s to indicate places where upai needed to remove
        data (NaNs) or felt uncertain about data (i.e. SVM and/or DI index)
        """

    # deprecated functions
    def data_cleaning_train(self, dk: UpaiDataKitchen, pair: str):
        """
        ...
        logger.warning(f"Your model {self.__class__.__name__} relies on the deprecated"
                       ...
                       f"{DOCS_LINK}/strategy_migration/#upai-new-data-pipeline")
    ...
    def data_cleaning_predict(self, dk: UpaiDataKitchen, pair: str):
        """
        throw deprecation warning if this function is called
        """
        logger.warning(f"Your model {self.__class__.__name__} relies on the deprecated"
        ...
        if self.upai_info.get("DI_threshold", 0) > 0:
