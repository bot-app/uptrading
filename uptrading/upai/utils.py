...
def download_all_data_for_training(dp: DataProvider, config: Config) -> None:
    ...
    refresh_backtest_ohlcv_data(
        dp._exchange,
        pairs=all_pairs,
        timeframes=config["upai"]["feature_parameters"].get("include_timeframes"),
...
def get_required_data_timerange(config: Config) -> TimeRange:
    ...
    for auto data-download in UpAi
    """
    ...
    indicator_periods = config["upai"]["feature_parameters"]["indicator_periods_candles"]
    ...
    logger.info(f'UpAi auto-downloader using {max_period} startup candles.')

    additional_seconds = max_period * max_tf_seconds

    startts = int(
        time
        - config["upai"].get("train_period_days", 0) * 86400
        - additional_seconds
    )
    stopts = int(time)
...
def plot_feature_importance(model: Any, pair: str, dk: UpaiDataKitchen,
    ...
    """
    from uptrading.plot.plotting import go, make_subplots, store_plot_file

    # Extract feature importance from model
    models = {}
    if 'UpaiMultiOutputRegressor' in str(model.__class__):
    ...
    for label in models:
        ...
        label = label.replace('&', '').replace('%', '')  # escape two UpAi specific characters
...
def get_timerange_backtest_live_models(config: Config) -> str:
    ...
    """
    dk = UpaiDataKitchen(config)
    models_path = dk.get_full_models_path(config)
    dd = UpaiDataDrawer(models_path, config)
...
def get_tb_logger(model_type: str, path: Path, activate: bool) -> Any:
    ...
    if model_type == "pytorch" and activate:
        from uptrading.upai.tensorboard import TBLogger
        return TBLogger(path, activate)
    else:
        from uptrading.upai.tensorboard.base_tensorboard import BaseTensorboardLogger
