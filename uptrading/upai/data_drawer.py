...
from uptrading.constants import Config
from uptrading.data.history import load_pair_history
...
class UpaiDataDrawer:
    ...
    def __init__(self, full_path: Path, config: Config):
        ...
        self.config = config
        self.upai_info = config.get("upai", {})
        ...
        self.empty_pair_dict: pair_info = {
                ...
                "data_path": "", "extras": {}}
        self.model_type = self.upai_info.get('model_save_type', 'joblib')
        ...
        if exists:
            try:
                ...
                with self.historic_predictions_bkp_path.open("rb") as fp:
                    self.historic_predictions = cloudpickle.load(fp)
                logger.warning('UpAi successfully loaded the backup historical predictions file.')
    ...
    def set_initial_return_values(self, pair: str,
        ...
        We also aim to keep the date from historical predictions so that the UpUI displays
        zeros during any downtime (between UpAi reloads).
        """
        ...
        if len(common_dates.index) > 0:
            ...
            logger.warning("No common dates found between new predictions and historic "
                           "predictions. You likely left your UpAi instance offline "
    ...
    def append_model_predictions(self, pair: str, predictions: DataFrame,
                                 ...
                                 dk: UpaiDataKitchen, strat_df: DataFrame) -> None:
        """
        Append model predictions to historic predictions dataframe, then set the
        strategy return dataframe to the tail of the historic predictions. The length of
        the tail is equivalent to the length of the dataframe that entered UpAi from
        the strategy originally. Doing this allows UpUI to always display the correct
    ...
    def return_null_values_to_strategy(self, dataframe: DataFrame, dk: UpaiDataKitchen) -> None:
        """
    ...
    def purge_old_models(self) -> None:
        ...
        num_keep = self.upai_info["purge_old_models"]
        if not num_keep:
            return
            ...
            if delete_dict[coin]["num_folders"] > num_keep:
                ...
                for k, v in sorted_dict.items():
                    if deleted >= num_delete:
                        break
                    logger.info(f"Upai purging old model file {v}")
                    shutil.rmtree(v)
                    deleted += 1

    def save_metadata(self, dk: UpaiDataKitchen) -> None:
        """
    ...
    def save_data(self, model: Any, coin: str, dk: UpaiDataKitchen) -> None:
        """
    ...
    def load_metadata(self, dk: UpaiDataKitchen) -> None:
        """
    ...
    def load_data(self, coin: str, dk: UpaiDataKitchen) -> Any:  # noqa: C901
        """
        ...
        if dk.live and coin in self.model_dictionary:
            ...
            mod = importlib.import_module(
                self.model_type, self.upai_info['rl_config']['model_type'])
            MODELCLASS = getattr(mod, self.upai_info['rl_config']['model_type'])
    ...
    def update_historic_data(self, strategy: IStrategy, dk: UpaiDataKitchen) -> None:
        """
        ...
        """
        feat_params = self.upai_info["feature_parameters"]
    ...
    def load_all_pair_histories(self, timerange: TimeRange, dk: UpaiDataKitchen) -> None:
        """
        ...
        for pair in dk.all_pairs:
            if pair not in history_data:
                history_data[pair] = {}
            for tf in self.upai_info["feature_parameters"].get("include_timeframes"):
    ...
    def get_base_and_corr_dataframes(
        self, timerange: TimeRange, pair: str, dk: UpaiDataKitchen
        ...
        with self.history_lock:
            ...
            pairs = self.upai_info["feature_parameters"].get(
    ...
    def get_timerange_from_live_historic_predictions(self) -> TimeRange:
        ...
        if not self.historic_predictions_path.is_file():
            raise OperationalException(
                ...
                'to run backtest with the upai-backtest-live-models option '
            )
