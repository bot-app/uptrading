...
from uptrading.constants import (CANCELED_EXCHANGE_STATES, CUSTOM_TAG_MAX_LENGTH,
                                 DATETIME_PRINT_FORMAT, MATH_CLOSE_PREC, NON_OPEN_EXCHANGE_STATES,
                                 BuySell, LongShort)
from uptrading.enums import ExitType, TradingMode
from uptrading.exceptions import DependencyException, OperationalException
from uptrading.exchange import (ROUND_DOWN, ROUND_UP, amount_to_contract_precision,
                                price_to_precision)
from uptrading.leverage import interest
from uptrading.misc import safe_value_fallback
from uptrading.persistence.base import ModelBase, SessionType
from uptrading.util import FtPrecise, dt_from_ts, dt_now, dt_ts
...
class LocalTrade:
    ...
    def calc_close_trade_value(self, rate: float, amount: Optional[float] = None) -> float:
        ...
        if trading_mode == TradingMode.SPOT:
            ...
            raise OperationalException(
                f"{self.trading_mode.value} trading is not yet available using uptrading")
