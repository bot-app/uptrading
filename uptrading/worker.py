...
from uptrading.configuration import Configuration
from uptrading.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from uptrading.enums import RPCMessageType, State
from uptrading.exceptions import OperationalException, TemporaryError
from uptrading.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.uptrading = UptradingBot(self._config)
    ...
    def _worker(self, old_state: Optional[State]) -> State:
        ...
        """
        state = self.uptrading.state

        # Log state transition
        if state != old_state:

            if old_state != State.RELOAD_CONFIG:
                self.uptrading.notify_status(f'{state.name.lower()}')

            logger.info(
                f"Changing state{f' from {old_state.name}' if old_state else ''} to: {state.name}")
            if state == State.RUNNING:
                self.uptrading.startup()

            if state == State.STOPPED:
                self.uptrading.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.uptrading.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.uptrading.process_stopped()

    def _process_running(self) -> None:
        try:
            self.uptrading.process()
            ...
            self.uptrading.notify_status(
                f'*OperationalException:*\n```\n{tb}```\n {hint}',
                msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception('OperationalException. Stopping trader ...')
            self.uptrading.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current uptradingbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current uptrading modules
        self.uptrading.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.uptrading.notify_status('config reloaded')
    ...
    def exit(self) -> None:
        ...
        if self.uptrading:
            self.uptrading.notify_status('process died')
            self.uptrading.cleanup()