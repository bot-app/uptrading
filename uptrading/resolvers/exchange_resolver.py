...
import uptrading.exchange as exchanges
from uptrading.constants import Config, ExchangeConfig
from uptrading.exchange import MAP_EXCHANGE_CHILDCLASS, Exchange
...
class ExchangeResolver(IResolver):
    ...
    def _load_exchange(exchange_name: str, kwargs: dict) -> Exchange:
        ...
        Only checks for exchanges exported in uptrading.exchanges
        ...
        for exchange_name in dir(exchanges):
            ...
            if isclass(exchange) and issubclass(exchange, Exchange):
                result.append({
                    ...
                    'location_rel: ': exchange.__module__.replace('uptrading.', ''),
                })
        return result
