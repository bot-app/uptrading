...
from uptrading.configuration.deprecated_settings import process_deprecated_setting
from uptrading.enums import RunMode, TradingMode
...
def _extend_validator(validator_class):
    """
    Extended validator for the Uptrading configuration JSON Schema.
...
UptradingValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: Dict[str, Any], preliminary: bool = False) -> Dict[str, Any]:
    ...
    try:
        UptradingValidator(conf_schema).validate(conf)
        return conf
...
def validate_config_consistency(conf: Dict[str, Any], preliminary: bool = False) -> None:
    ...
    _validate_upai_hyperopt(conf)
    _validate_upai_backtest(conf)
    _validate_upai_include_timeframes(conf)
...
def _validate_upai_hyperopt(conf: Dict[str, Any]) -> None:
    upai_enabled = conf.get('upai', {}).get('enabled', False)
    analyze_per_epoch = conf.get('analyze_per_epoch', False)
    if analyze_per_epoch and upai_enabled:
        raise OperationalException(
            'Using analyze-per-epoch parameter is not supported with a UpAi strategy.')


def _validate_upai_include_timeframes(conf: Dict[str, Any]) -> None:
    upai_enabled = conf.get('upai', {}).get('enabled', False)
    if upai_enabled:
        main_tf = conf.get('timeframe', '5m')
        upai_include_timeframes = conf.get('upai', {}).get('feature_parameters', {}
                                                               ).get('include_timeframes', [])

        from uptrading.exchange import timeframe_to_seconds
        main_tf_s = timeframe_to_seconds(main_tf)
        offending_lines = []
        for tf in upai_include_timeframes:
            tf_s = timeframe_to_seconds(tf)
            if tf_s < main_tf_s:
                offending_lines.append(tf)
        if offending_lines:
            raise OperationalException(
                f"Main timeframe of {main_tf} must be smaller or equal to UpAi "
                f"`include_timeframes`.Offending include-timeframes: {', '.join(offending_lines)}")

        # Ensure that the base timeframe is included in the include_timeframes list
        if main_tf not in upai_include_timeframes:
            feature_parameters = conf.get('upai', {}).get('feature_parameters', {})
            include_timeframes = [main_tf] + upai_include_timeframes
            conf.get('upai', {}).get('feature_parameters', {}) \
                .update({**feature_parameters, 'include_timeframes': include_timeframes})


def _validate_upai_backtest(conf: Dict[str, Any]) -> None:
    if conf.get('runmode', RunMode.OTHER) == RunMode.BACKTEST:
        upai_enabled = conf.get('upai', {}).get('enabled', False)
        timerange = conf.get('timerange')
        upai_backtest_live_models = conf.get('upai_backtest_live_models', False)
        if upai_backtest_live_models and upai_enabled and timerange:
            raise OperationalException(
                'Using timerange parameter is not supported with '
                '--upai-backtest-live-models parameter.')

        if upai_backtest_live_models and not upai_enabled:
            raise OperationalException(
                'Using --upai-backtest-live-models parameter is only '
                'supported with a UpAi strategy.')

        if upai_enabled and not upai_backtest_live_models and not timerange:
            raise OperationalException(
                'Please pass --timerange if you intend to use UpAi for backtesting.')
