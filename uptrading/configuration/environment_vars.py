...
from uptrading.constants import ENV_VAR_PREFIX
from uptrading.misc import deep_merge_dicts
...
def flat_vars_to_nested_dict(env_dict: Dict[str, Any], prefix: str) -> Dict[str, Any]:
    """
    Environment variables must be prefixed with UPTRADING.
    UPTRADING__{section}__{key}
    :param env_dict: Dictionary to validate - usually os.environ
    :param prefix: Prefix to consider (usually UPTRADING__)
...
def enironment_vars_to_dict() -> Dict[str, Any]:
    ...
    Relevant variables must follow the UPTRADING__{section}__{key} pattern
    :return: Nested dict based on available and relevant variables.
    """
