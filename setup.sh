...
function updateenv() {
    ...
    then
        ...
        read -p "Do you want to install dependencies for upai [y/N]? "
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
            REQUIREMENTS_UPAI="-r requirements-upai.txt --use-pep517"
            read -p "Do you also want dependencies for upai-rl or PyTorch (~700mb additional space required) [y/N]? "
            if [[ $REPLY =~ ^[Yy]$ ]]
            then
                REQUIREMENTS_UPAI="-r requirements-upai-rl.txt"
            fi
        fi
    fi
    install_talib

    ${PYTHON} -m pip install --upgrade -r ${REQUIREMENTS} ${REQUIREMENTS_HYPEROPT} ${REQUIREMENTS_PLOT} ${REQUIREMENTS_UPAI} ${REQUIREMENTS_UPAI_RL}
    if [ $? -ne 0 ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing Uptrading"
        exit 1
    fi

    echo "Installing upUI"
    uptrading install-ui
...
function config() {
    echo_block "Please use 'uptrading new-config -c user_data/config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .venv/bin/activate; uptrading <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .venv/bin/activate; uptrading --help'."
    echo "You verify that uptrading is installed successfully by running 'source .venv/bin/activate; uptrading --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install uptrading from scratch"
