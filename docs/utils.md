...
Creates the directory structure to hold your files for uptrading.
...
```
usage: uptrading create-userdir [-h] [--userdir PATH] [--reset]
...
```
usage: uptrading new-config [-h] [-c PATH]
...
!!! Warning
    Only vital questions are asked. Uptrading offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ uptrading new-config --config config_binance.json
...
``` output
usage: uptrading new-strategy [-h] [--userdir PATH] [-s NAME]
...
```bash
uptrading new-strategy --strategy AwesomeStrategy
```

With custom user directory

```bash
uptrading new-strategy --userdir ~/.uptrading/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
uptrading new-strategy --strategy AwesomeStrategy --template advanced
```
...
```
usage: uptrading list-strategies [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
...
``` bash
uptrading list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
uptrading list-strategies --userdir ~/.uptrading/
```

Example: Search dedicated strategy path.

``` bash
uptrading list-strategies --strategy-path ~/.uptrading/strategies/
```
...
```
usage: uptrading list-freqaimodels [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                   [-d PATH] [--userdir PATH]
...
```
usage: uptrading list-exchanges [-h] [-1] [-a]
...
```
$ uptrading list-exchanges
Exchanges available for Uptrading:
...
* Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with Uptrading):
```
$ uptrading list-exchanges -a
...
```
usage: uptrading list-timeframes [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH] [--userdir PATH] [--exchange EXCHANGE] [-1]
...
```
$ uptrading list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for Uptrading and print timeframes supported by each of them:
```
$ for i in `uptrading list-exchanges -1`; do uptrading list-timeframes --exchange $i; done
```
...
For pairs traded by Uptrading the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
usage: uptrading list-markets [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
usage: uptrading list-pairs [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                            [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                            [--print-list] [--print-json] [-1] [--print-csv]
                            [--base BASE_CURRENCY [BASE_CURRENCY ...]]
                            [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]] [-a]
...
```
$ uptrading list-pairs --quote USD --print-json
```
...
```
$ uptrading list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ uptrading list-markets --exchange kraken --all
```
...
```
usage: uptrading test-pairlist [-h] [--userdir PATH] [-v] [-c PATH]
                               [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
...
```
uptrading test-pairlist --config config.json --quote USDT BTC
```

## Convert database

`uptrading convert-db` can be used to convert your database from one system to another (sqlite -> postgres, postgres -> other postgres), migrating all trades, orders and Pairlocks.
...
```
usage: uptrading convert-db [-h] [--db-url PATH] [--db-url-from PATH]
...
!!! Warning
    Please ensure to only use this on an empty target database. Uptrading will perform a regular migration, but may fail if entries already existed.
...
Run uptrading in webserver mode.
Uptrading will start the webserver and allow FreqUI to start and control backtesting processes.
...
```
usage: uptrading webserver [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
                           [--userdir PATH]
...
You can use `docker compose run --rm -p 127.0.0.1:8080:8080 uptrading webserver` to start a one-off container that'll be removed once you stop it. This assumes that port 8080 is still available and no other bot is running on that port.
...
```
usage: uptrading backtesting-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
...
```
usage: uptrading backtesting-analysis [-h] [-v] [--logfile FILE] [-V]
...
```
usage: uptrading hyperopt-list [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
```
uptrading hyperopt-list --profitable --no-details
```
...
```
usage: uptrading hyperopt-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH] [--best]
...
```
uptrading hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
uptrading hyperopt-show --best -n -1 --print-json --no-header
```
...
```
usage: uptrading show-trades [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
uptrading show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
...
```
usage: uptrading strategy-updater [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
